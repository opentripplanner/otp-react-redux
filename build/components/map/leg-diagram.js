...
var _coreUtils$itinerary = _coreUtils.default.itinerary,
    getElevationProfile = _coreUtils$itinerary.getElevationProfile,
    getTextWidth = _coreUtils$itinerary.getTextWidth,
    legElevationAtDistance = _coreUtils$itinerary.legElevationAtDistance; // Fixed dimensions for chart
...
function (_Component) {
  ...
  function LegDiagram(props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "_getElevationProfile", (0, _lodash.default)(function (leg) {
      if (!leg) return {};
      return getElevationProfile(leg.steps, _this._unitConversion());
    }));
  ...
  _createClass(LegDiagram, [{
    ...
    value: function render() {
      ...
      if (elevationPoint) {
        var _elev = legElevationAtDistance(points, elevationPoint);
        ...
        for (var i = 0; i < stepArr.length; i++) {
          if (x >= stepArr[i] && x <= stepArr[i + 1]) {
            ...
            var anchor = 'middle';
            var _x = midPoint;
            var halfLabelWidth = getTextWidth(name) / 2;
