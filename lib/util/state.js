...
// time, etc.
const FARE_FACTOR = 0.5
const DURATION_FACTOR = 0.25
const WALK_RELUCTANCE = 0.1
const WAIT_RELUCTANCE = 0.1
const DRIVE_RELUCTANCE = 2
const TRANSFER_RELUCTANCE = 0.9
...
function calculateItineraryCost (itinerary, config = {}) {
  const weights = {
    ...
    driveReluctance: DRIVE_RELUCTANCE,
    durationFactor: DURATION_FACTOR,
    fareFactor: FARE_FACTOR,
    waitReluctance: WAIT_RELUCTANCE,
    transferReluctance: TRANSFER_RELUCTANCE
  }
...
function getTotalFare (itinerary, config = {}) {
  ...
  // Start with default weighting values.
  const costs = {
    bikeshareTripCostCents: DEFAULT_BIKESHARE_FARE,
    carParkingCostCents: DEFAULT_PARKING_COST,
    drivingCentsPerMile: DEFAULT_DRIVING_COST_PER_MILE
  }
  // If config contains values to override defaults, apply those.
