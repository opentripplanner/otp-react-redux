...
function getActiveSearchQuery (state) {
  const activeSearch = getActiveSearch(state.otp)
  return activeSearch && activeSearch.query
}
...
export const getTransitiveData = createSelector(
  ...
  (
    ...
    if (hasResponse) {
      if (query.routingType === 'ITINERARY' && hasAtLeastOneResponse) {
        const visibleIndex = visibleItinerary !== undefined && visibleItinerary !== null
          ? visibleItinerary
          : activeItinerary
        const foundItinerary = itins[visibleIndex] || activeItinerary
