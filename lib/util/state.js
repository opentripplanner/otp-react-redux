...
export function getOperatorName(operator, agencyName) {
  return operator?.name || agencyName
}
...
export function getOperatorAndRoute(routeObject, transitOperators, intl) {
  ...
  const { agency, agencyName, longName, shortName } = routeObject
  const operatorName = getOperatorName(operator, agency?.name || agencyName)
