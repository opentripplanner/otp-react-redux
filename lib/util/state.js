...
export const getActiveItineraries = createSelector(
  ...
  (config, nonRealtimeResponse, realtimeResponse) => {
    ...
    // Field trip needs to hide all itineraries that don't have a group size
    if (config?.modules?.find((m) => m.id === 'ft')) {
      return itineraries.filter((i) => i?.fieldTripGroupSize > 0)
    }
    return itineraries
  }
)
