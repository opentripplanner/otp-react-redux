...
import { summarizeQuery } from '../components/form/user-settings-i18n'
...
/** Helper method to get the user-friendly operator name (fallback on GTFS agency name). */
export function getOperatorName(operator, agency) {
  return operator?.name || agency.name
}

export function getRouteOperatorName(routeObject, transitOperators) {
  const operator = getRouteOperator(routeObject, transitOperators)
  return getOperatorName(operator, routeObject.agency)
}
