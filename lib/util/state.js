...
import { isModuleEnabled, Modules } from './config'
...
/**
 * Returns the config of the specified module if it exists.
 *
 * @param  {Object} config   the app-wide config
 * @param  {string} moduleId the desired id of the module
 */
function getModuleConfig (config, moduleId) {
  return config.modules?.find(moduleConfig => moduleConfig.id === moduleId)
}
 ...
 */
const getFieldTripGroupCapacityForMode = createSelector(
  config => getModuleConfig(config, 'ft')?.modeCapacities,
  (config, mode) => mode,
...
function calculateItineraryFieldTripGroupCapacity ({
  ...
  return itinerary.legs.reduce((constrainingLegCapacity, leg) => {
    if (!leg.transitLeg) {
      return constrainingLegCapacity
    }
    return Math.min(
      constrainingLegCapacity,
      getFieldTripGroupCapacityForMode(config, leg.mode)
    )
  }, 10000)
}
...
export const getActiveItineraries = createSelector(
  ...
  state => isModuleEnabled(state, Modules.FIELD_TRIP),
