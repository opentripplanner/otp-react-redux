...
import { getActiveFieldTripRequest } from './call-taker'
...
export function getErrorMessage (error, errorMessages, locale) {
  ...
  if (error.network) {
    ...
    const noTripFoundText = (error.showModes && error.modes.length > 0)
      ? (
        <FormattedMessage
            ...
            modes: (
              <FormattedList
                ...
                value={error.modes.map(
                  ...
                  mode => <StyledFormattedMode key={mode} mode={mode.toLowerCase()} />
                )}
              />
            )
          }}
  ...
  if (errorMessages) {
    ...
    if (msgConfig) {
      ...
      const { modes, msg } = msgConfig
      // If the message is defined for the exact locale or the language, use it,
      // otherwise fall back to the top-level message that applies to all locales.
      const localizedMessage = msg && (msg[locale] || msg[lang] || msg)
      if (error.modes && modes) {
...
function calculateItineraryCost (itinerary, config = {}) {
  ...
  return getTotalFare(itinerary, config.itinerary?.costs, config.itinerary?.defaultFareKey) * weights.fareFactor +
 ...
 * TODO: Add GBFS fares
 */
export function getTotalFare (itinerary, configCosts = {}, defaultFareKey = 'regular') {
  ...
  (routes, filter) =>
    routes.filter(
      (route) =>
        ...
        (!filter.search ||
          ((route.longName && route.longName.toLowerCase().includes(filter.search.toLowerCase())) ||
            (route.shortName && route.shortName.toLowerCase().includes(filter.search.toLowerCase()))))
    )
)
...
export const getSortedFilteredRoutes = createSelector(
  ...
  (routes, transitOperators) => routes.sort(
...
export const getModesForActiveAgencyFilter = createSelector(
  routeSelector,
  routeViewerFilterSelector,
  (routes, filter) => Array.from(
    new Set(
      routes
        .filter(route => (route.mode && (!filter.agency || filter.agency === route.agencyName)))
 ...
 */
export const getAgenciesFromRoutes = createSelector(
  routeSelector,
  (routes) => Array.from(
...
export function getTitle (state, intl) {
  ...
  switch (mainPanelContent) {
    ...
    default:
      const activeSearch = getActiveSearch(state)
      if (activeSearch) {
        status = coreUtils.query.summarizeQuery(activeSearch.query, user.locations)
      }
      break
  }
