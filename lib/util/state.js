...
import { getModuleConfig, Modules } from './config'
 ...
 *
 * @param  {Object} param
 * @param  {Object} param.fieldTripModuleConfig The field trip module config
...
function calculateItineraryFieldTripGroupCapacity ({
  ...
  return itinerary.legs.reduce((constrainingLegCapacity, leg) => {
    ...
    return Math.min(
      ...
      getFieldTripGroupCapacityForMode(fieldTripModuleConfig, leg.mode)
    )
  }, 10000)
}
...
export const getActiveItineraries = createSelector(
  ...
  state => getModuleConfig(state, Modules.FIELD_TRIP),
