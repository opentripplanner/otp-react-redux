...
/**
 * If there are transit itineraries that are faster than walking,
 * ensure that the error WALKING_BETTER_THAN_TRANSIT is not shown.
 */
function removeWalkFasterThanTransitIfNeeded(itineraries, errorsByCode) {
  if (errorsByCode.WALKING_BETTER_THAN_TRANSIT) {
    const transitItineraries = itineraries.filter((itin) =>
      itin.legs.some(coreUtils.itinerary.isTransitLeg)
    )

    const transitItineraryTimes = transitItineraries
      .map((itin) => itin.duration)
      .sort()
    const minTransitTime = transitItineraryTimes[0]

    const walkItinerary = itineraries.find(
      (itin) => itin.legs.length === 1 && itin.legs[0].mode === 'WALK'
    )

    if (walkItinerary.duration > minTransitTime) {
      delete errorsByCode.WALKING_BETTER_THAN_TRANSIT
    }
  }
}
...
export function getActiveSearchErrors(state) {
  ...
  removeWalkFasterThanTransitIfNeeded(itineraries, errorsByCode)
