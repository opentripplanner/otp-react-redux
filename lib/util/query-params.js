    ...
    applicable: (query, config) => {
      ...
      for (let mode of query.mode.split(',')) {
        if (config.modes && config.modes.accessModes) {
          const accessMode = config.modes.accessModes.find(m => m.mode === mode)
          if (accessMode && accessMode.showWheelchairSetting) return true
        }
        if (config.modes && config.modes.transitModes) {
          const transitMode = config.modes.transitModes.find(m => m.mode === mode)
          if (transitMode && transitMode.showWheelchairSetting) return true
        }
      }
    }
  }
