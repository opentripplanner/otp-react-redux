...
const queryParams = [
  ...
  { /* optimize -- how to optimize an itinerary trip */
    name: 'optimize',
    applicable: query => hasBike(query.mode) || hasTransit(query.mode),
    routingTypes: [ 'ITINERARY' ],
    default: 'QUICK',
    selector: 'DROPDOWN',
    label: 'Optimize for',
    options: query => {
      // Always inclide the QUICK option
      let opts = [{
        text: 'Speed',
        value: 'QUICK'
      }]

      // Add transit-specific options, if applicable
      ...
      // Add bike-specific options, if applicable
      if (hasBike(query.mode)) {
        opts = opts.concat([{
