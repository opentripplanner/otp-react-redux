  ...
  return {
    ...
    onRedirectCallback: appState => {
      if (appState && appState.urlHash) {
        // Immediately after successful login, Auth0 redirects to /signedin (Auth0-whitelisted).
        // Here, we add back the search query params that are contained within urlHash to preserve any itinerary search
        // that may have been present before login (and avoid a URL without a search showing in the user's browsing history).
        const nextUrl = `${URL_ROOT}${appState.urlHash.replace('#/', '#/signedin')}`
