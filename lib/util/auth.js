  ...
  return {
    ...
    onRedirectCallback: appState => {
      if (appState && appState.urlHash) {
        ...
        // This code block adds the the hash parameters to /singedin,
        // so that if the user has set up their MOD UI account, the handler for /signedin
        // can route the browser to the itinerary search just before login (stored in appState.urlHash).
        // This code block prevents the URL with no search from appearing in the user's browsing history.
        const newUserPath = appState.urlHash.replace('#/', '#/signedin')
