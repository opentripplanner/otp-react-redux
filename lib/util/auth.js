  ...
  return {
    ...
    onRedirectCallback: appState => {
      if (appState && appState.urlHash) {
        // Immediately after successful login, Auth0 will redirect to
        // /signedin (Auth0-whitelisted) with no search parameters.
        // This code block redirects to the /signedinswitch URL and pass the hash parameters,
        // so that if the user has set up their MOD UI account, the handler for /signedinswitch
        // can route the browser to the state just before login (stored in appState.urlHash).
        // This code block prevents the URL with no search from appearing in the user's browsing history.
        const newUserPath = appState.urlHash.replace('#/', '#/signedinswitch')
