...
export function createLetter (formData, mailablesConfig) {
  const imageUrl = mailablesConfig.header_graphic
  // A valid URL must be provided in the config in order for the create letter
  // approach to function properly.
  if (!imageUrl) {
    return alert(`Error constructing letter. Mailables headerGraphic config property is missing.`)
  }
...
function writePDF (formData, imageData, otpConfig) {
  ...
  doc.on('pageAdded', () => preparePage(doc, otpConfig))

  // current date
  doc.text(moment().format('MMMM D, YYYY'))
  ...
  const tableData = {
    ...
    rows: mailables.map(mailable => {
      let {name, largeFormat, largePrint, quantity} = mailable
...
function preparePage (doc, imageData, otpConfig) {
  const {footer, headerGraphic, headerGraphicHeight, headerGraphicWidth} = otpConfig
  ...
  if (footer) {
    doc.fontSize(9)
      .text(
        ...
        doc.page.height - 50,
        {align: 'center', lineBreak: false, width: 500}
      )
  }
  // Add header image.
  if (headerGraphic) {
