...
export function parseDate(date) {
  return parse(date, ENTITY_DATE_TIME_FORMAT, referenceDateForDateFns)
}

/**
 * Helper function to convert a date from the browser's timezone to the specified one.
 */
export function toTimeZone(date, timeZone) {
  const dateUtc = zonedTimeToUtc(date, getUserTimezone())
      ...
      includedInSearch = searchTerms.every((term) => {
        ...
        if (splitBySlash.length > 1) {
          ...
          if (!isNaN(month) && !isNaN(day)) {
            ...
            const date = toTimeZone(
              parseDate(request.travelDate),
              otp.config.homeTimezone
            )
