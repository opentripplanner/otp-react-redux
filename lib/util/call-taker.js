import { compareAsc, differenceInCalendarDays, parse } from 'date-fns'
...
import { getISOLikeTimestamp } from './state'
const ENTITY_DATE_TIME_FORMAT = 'MMM d, yyyy h:mm:ss a'

/**
 * Only used in parseDate below.
 */
...
function parseDate(date) {
  parse(date, ENTITY_DATE_TIME_FORMAT, referenceDateForDateFns)
}
      ...
      includedInSearch = searchTerms.every((term) => {
        ...
        if (splitBySlash.length > 1) {
          ...
          if (!isNaN(month) && !isNaN(day)) {
            ...
            // in the the configured homeTimezone.
            const dateInBrowserTz = parseDate(request.travelDate)
            const isoDateWithoutTz = getISOLikeTimestamp(
              getUserTimezone(),
              dateInBrowserTz
            )
            const date = toDate(isoDateWithoutTz, {
              timeZone: otp.config.homeTimezone
            })
