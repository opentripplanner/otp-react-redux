export function filterProfileOptions (options) {
...
export function profileOptionsToItineraries (options) {
  return options.map(option => optionToItinerary(option))
}

// helper functions for profileOptionsToItineraries:

function optionToItinerary (option) {
  const itin = {
  ...
  if (option.access && option.access.length > 0) {
    if (option.access[0].mode === 'BICYCLE_RENT') {
      ...
      itin.legs.push(accessToLeg(option.access[0]))
  ...
  if (option.egress && option.egress.length > 0) {
    ...
    itin.legs.push(accessToLeg(option.egress[0], origin))
