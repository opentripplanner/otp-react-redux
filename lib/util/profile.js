export function filterProfileOptions (response) {
  ...
  const filteredProfile = response.profile.filter((option, i) => {
  ...
  const filteredJourneys = response.journeys.filter((journey, i) => filteredIndices.indexOf(i) !== -1)

  return Object.assign(response, { profile: filteredProfile, journeys: filteredJourneys })
}
