...
function getOtpUiPackagesForI18n() {
  return [
    'base-map',
    'building-blocks',
    'endpoints-overlay',
...
async function loadOtpUiLocaleData(matchedLocale) {
  const otpUiMessages = await Promise.all(
    getOtpUiPackagesForI18n()
      .map(async (pkg) => {
        try {
          const msgs = await import(
            // In Vite, dynamic imports with variables must use a relative path, even with libraries.
            `../../node_modules/@opentripplanner/${pkg}/i18n/${matchedLocale}.yml`
          )
...
export async function loadLocaleData(matchedLocale, customMessages) {
  ...
  // Vite-specific dynamic import with variable.
  const messages = await import(`../../i18n/${loadedLocale}.yml`)
