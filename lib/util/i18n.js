 ...
 */
export async function loadLocaleData(locale, customMessages, configLocales) {
  const matchedLocale = getMatchingLocaleString(locale, 'en-US', configLocales)
  let messages
  switch (matchedLocale) {
    case 'es-ES':
      messages = await import('../../i18n/es-ES.yml')
      break
    case 'fr-FR':
      messages = await import('../../i18n/fr-FR.yml')
      break
    case 'ko-KR':
      messages = await import('../../i18n/ko-KR.yml')
      break
    case 'vi-VN':
      messages = await import('../../i18n/vi-VN.yml')
      break
    case 'zh-CN':
      messages = await import('../../i18n/zh-CN.yml')
      break
    default:
