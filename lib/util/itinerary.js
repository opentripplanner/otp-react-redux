 ...
 *
 * IMPORTANT: both a and b must be numeric values or at least be attempted to
 * be converted to numeric values! If one of the arguments is something crazy
 * like an empty string, unexpected behavior will occur because JavaScript.
 */
function validNumericValueComparator (a, b) {
  if (isNaN(a)) {
    // a is not a number
    if (isNaN(b)) {
      // a and b are not numbers, therefore a secondary sorting criteria should
      // be used. Return undefined to indicate this
      return undefined
    }
  ...
  // a is a valid number
  if (isNaN(b)) {
    // b is not a number, therefore a gets priority
    return -1
  }
 ...
 * interface. Due to GTFS feeds having varying levels of data quality, a
 * multi-criteria sort is needed to account for various differences. The sort
 * occurs as follows:
 * 1. Attempt to sort based off of routeSortOrder. Routes that do not have a
 *    valid routeSortOrder will be placed beneath those that do.
 * 2. In case of the same routeSortOrder criteria, attempt to sort based off of
 *    routeShortNames that are parseable as integers. routeShortNames that
 ...
 *    undefined.
 *
 * @param  {Object} a Route A
 * @param  {Object} b Route B
 * @return {number}   A numeric sort comparison value
 */
export function routeComparator (a, b) {
  // If route sort order field is available, default sort to these predefined
  // values.
  const routeSortOrderComparatorValue = validNumericValueComparator(
    getRouteSortOrderValue(a.sortOrder),
    getRouteSortOrderValue(b.sortOrder)
  )

  // if a valid sort order comparison value was able to be obtained, return that
  // value. Otherwise, attempt next sort criteria
  if (routeSortOrderComparatorValue !== undefined) {
    return routeSortOrderComparatorValue
  }

  // Otherwise, if both short names can be parsed as integers, use these
  // numbers for sorting.
  const intShortNameComparatorValue = validNumericValueComparator(
    parseInt(a.shortName),
    parseInt(b.shortName)
  )

  // if a valid sort order comparison value was able to be obtained, return that
  // value. Otherwise, attempt next sort criteria
  if (intShortNameComparatorValue !== undefined) {
    return intShortNameComparatorValue
  }

  // Otherwise, default to short or long name strings.
  const aComp = a.shortName || a.longName
  const bComp = b.shortName || b.longName
