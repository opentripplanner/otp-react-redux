 ...
 *  2. route type (OTP mode). The priority is as follows:
 *    - SUBWAY
 *    - RAIL
 *    - TRAM
 *    - CABLE_CAR
 ...
 *    - FUNICULAR
 *    - BUS
 *  3. shortName as integer. shortNames that cannot be parsed as integers will
 *    be placed beneath those that are valid.
 *  4. shortName as string. Routes without shortNames will be placed beneath
 *    those with shortNames.
 *  5. longName as string.
 *
 * @param  {Boolean} [sortOnIntegerShortName=true] Set to false to omit integer
 *  shortName criteria sort
 */
export function makeRouteComparator (sortOnIntegerShortName = true) {
  const criteria = [
    makeNumericValueComparator(obj => getRouteSortOrderValue(obj.sortOrder)),
    routeTypeComparator
  ]
  if (sortOnIntegerShortName) {
    criteria.push(makeNumericValueComparator(obj => parseInt(obj.shortName)))
  }
  criteria.push(makeStringValueComparator(obj => obj.shortName))
  criteria.push(makeStringValueComparator(obj => obj.longName))
  return makeMultiCriteriaSort(...criteria)
}
