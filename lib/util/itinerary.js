 ...
 * JavaScript.
 *
 * @param  {function} [objGetterFn] A function to obtain the comparison value
 *  from the comparator function arguments
 */
function makeNumericValueComparator (objGetterFn) {
  return (a, b) => {
    let aVal
    let bVal
    if (typeof objGetterFn === 'function') {
      aVal = objGetterFn(a)
      bVal = objGetterFn(b)
    } else {
      aVal = a
 ...
 *
 * @param  {string} key The key from which to obtain the sort comparison value
 */
function makeObjectStringValueComparator (key) {
  return (a, b) => {
    const aVal = a[key]
    const bVal = b[key]
 ...
 * occurs as follows:
 * 1. Attempt to sort based off of sortOrder. Routes that do not have a valid
 *    sortOrder will be placed beneath those that do.
 * 2. In case of the same sortOrder criteria, attempt to sort based off of
 *    shortNames that are parseable as integers. shortNames that cannot be
 *    parsed as integers will be placed beneath those that are valid.
 * 3. In case of the same integer shortName criteria, attempt to sort based
 *    off of the string shortName. Routes without shortNames will be placed
 *    beneath those with shortNames.
 * 4. In case of the same string shortName, attempt to sort based off of
 *    the string longName.
 *
...
export const routeComparator = makeMultiCriteriaSort(
  ...
  makeObjectStringValueComparator('shortName'),
  makeObjectStringValueComparator('longName')
)
