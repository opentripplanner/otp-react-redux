...
export function getColorAndNameFromRoute(operator, route) {
  const { defaultRouteColor, longNameSplitter } = operator || {}
  ...
  const softColor = tinycolor
    .mostReadable('#fff', [
      routeColor,
      tinycolor(routeColor).brighten(10).toHexString(),
      tinycolor(routeColor).brighten(20).toHexString(),
      tinycolor(routeColor).darken(10).toHexString()
    ])
    .toHexString()
...
export function getRouteColorBasedOnSettings(operator = {}, route = {}) {
  const { routeColor, softColor } = getColorAndNameFromRoute(operator, route)
