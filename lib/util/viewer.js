...
export function containsRealtimeLeg(itinerary) {
  return !!itinerary.legs.find((leg) => leg?.realTime)
}

/**
 * Helper method that returns true only if an itinerary's first leg
 * has realtime info
 */
export function firstLegIsRealtime(itinerary) {
  return itinerary.legs?.[0]?.realTime
}
 ...
 */
function getContrastYIQ(routeColor) {
