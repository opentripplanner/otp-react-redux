...
// Match against Pattern#desc to find the trip headsign. Pattern#desc looks like:
// - "513 to North Ave Station (1:12345)" or
// - "513 to North Ave Station (1:12345) from Midtown Station (1:678910)"
// Our regex grabs the "North Ave Station" bit.
//
const HEADSIGN_REGEX = new RegExp(/ to (.+?)(?= \(.*:.*\))/, 'g')
...
export function getStopTimesByPattern (stopData) {
  ...
  if (stopData && stopData.routes && stopData.stopTimes) {
    stopData.stopTimes.forEach(({ pattern, times }) => {
      ...
      let headsign = times[0]?.headsign
      if (isBlank(headsign)) {
        const matches = HEADSIGN_REGEX.exec(pattern.desc)
        if (matches) {
          headsign = matches[1]
        }
        // Reset regex's lastIndex so that next match attempt is not thwarted
        // (why, JavaScript? why?):
        // https://stackoverflow.com/a/21123303/915811
        HEADSIGN_REGEX.lastIndex = 0
        console.log(times[0]?.headsign, matches, pattern.desc)
      }
