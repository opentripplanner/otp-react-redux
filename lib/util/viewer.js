 ...
 */
export function getFormattedStopTime (stopTime, homeTimezone, soonText = 'Due', timeFormat, useSchedule = false, showIcon = true, useCountdown = true) {
  const departureTime = useSchedule
    ? stopTime.scheduledDeparture
    : stopTime.realtimeDeparture
 ...
 */
export function excludeLastStop ({ stopIndex, stopCount }) {
...
export function getStopTimesByPattern (stopData) {
  ...
  if (stopData && stopData.routes && stopData.stopTimes) {
    stopData.stopTimes.forEach(patternTimes => {
      ...
      const filteredTimes = patternTimes.times.filter(excludeLastStop)
...
/**
 * Compares departure times of two stops.
 */
