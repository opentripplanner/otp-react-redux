...
export function extractHeadsignFromPattern(pattern, routeShortName = null) {
  const { desc, headsign: originalHeadsign, stops } = pattern

  let headsign = originalHeadsign

  // Remove leading "To " text from headsign (English-specific)
  if (
    !isBlank(headsign) &&
    (headsign.startsWith('To ') || headsign.startsWith('to '))
  ) {
    headsign = headsign.substring(3)
  }
  ...
  if (isBlank(headsign)) {
    const matches = desc?.match(/ to ([^(from)]+) \(.+\)/)
    if (matches) {
      headsign = matches[1]
    }
  }
   ...
   */
  const descNotSameAsRouteShortName =
    routeShortName && desc !== `${routeShortName}`
  if (isBlank(headsign) && descNotSameAsRouteShortName) {
  ...
  // If the headsign is still blank, show the description if different than the route short name.
  if (isBlank(headsign) && descNotSameAsRouteShortName) {
    headsign = desc || ''
  }

  // If nothing else works, use the name of the last stop in the pattern.
  if (isBlank(headsign) && stops.length) {
...
export const getPatternViewerColors = (
  useRouteColorAsBackground,
  operator,
  route
) => {
  const routeColor = getRouteColorBasedOnSettings(operator, route)
  const backgroundColor = useRouteColorAsBackground ? routeColor : '#fff'
  const textColor = useRouteColorAsBackground
    ? getMostReadableTextColor(backgroundColor, route?.textColor)
    : DARK_TEXT_GREY
  return { backgroundColor, routeColor, textColor }
}
