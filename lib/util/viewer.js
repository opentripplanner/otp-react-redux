...
export function extractHeadsignFromPattern(pattern, routeShortName = null) {
  ...
  if (!isBlank(headsign)) {
    ...
    const attempts = [
      // This attempt and the next are aimed at pattern descriptions of the form:
      // '49 to <Destination> (<destid>)[ from <Origin> (<originid)]'
      (d) => d?.match(/(?: to )(.*?)(?: \()/)?.[1],
    ...
    for (let i = 0; i < attempts.length; i++) {
      const newHeadsign = attempts[i](desc)
      if (!isBlank(newHeadsign)) {
        return newHeadsign
      }
    }
    return ''
