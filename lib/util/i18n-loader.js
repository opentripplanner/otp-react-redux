...
const i18nAccountModules = import.meta.glob('../../i18n/account/*.yml', {
  import: 'default'
})
const i18nMobilityModules = import.meta.glob(
  '../../i18n/mobilityprofile/*.yml',
  {
    import: 'default'
  }
)
...
const appModules = {
  account: i18nAccountModules,
  mobilityprofile: i18nMobilityModules
}
...
export async function loadLocaleData(
  ...
  // Load any modules next.
  for (let i = 0; i < modules.length; i++) {
    const moduleMessages = await appModules[modules[i]][
      `../../i18n/${modules[i]}/${loadedLocale}.yml`
    ]()
    mergedMessages = {
      ...mergedMessages,
      ...flatten(moduleMessages)
    }
  }
