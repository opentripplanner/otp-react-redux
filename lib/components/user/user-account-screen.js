...
class UserAccountScreen extends Component {
  ...
  render () {
    const { isEmailVerified, loggedInUser } = this.props

    // Capture the handler for use in the Formik callback (where `this` might mean something else.)
...
const mapStateToProps = (state, ownProps) => {
  const { isEmailVerified, loggedInUser } = state.user
  return {
    isEmailVerified,
    loggedInUser
  }
