...
function DurationOptions ({ defaultValue, minuteOptions }) {
  ...
  const options = minuteOptions.map(minutes => ({
    text: minutes === 60
      ? intl.formatMessage({id: 'components.TripNotificationsPane.oneHour'}, {minutes})
      : intl.formatMessage({id: 'common.time.tripDurationFormatZeroHours'}, {minutes}),
    value: minutes
  }))
  return <Options defaultValue={defaultValue} options={options} />
}
    ...
    if (areNotificationsDisabled) {
      notificationSettingsContent = (
        <Alert bsStyle='warning'>
          <p>
            <Icon name='exclamation-triangle' withSpace />
            <strong>
      ...
      notificationSettingsContent = (
        <>
            ...
            <FormattedMessage
              ...
              values={
                notificationChannel === 'email'
                  ? {channel: <FormattedMessage id='common.notifications.email' />}
                  : {channel: <FormattedMessage id='common.notifications.sms' />}
              }
            />
          </h4>
          <SettingsListWithAlign>
            <li>
              <Select
                label={<FormattedMessage id='components.TripNotificationsPane.realtimeAlertFlagged' />}
                name='notifyOnAlert'
              >
            ...
            <li>
              <Select
                label={<FormattedMessage id='components.TripNotificationsPane.altRouteRecommended' />}
          ...
          <h4>
            <SettingsToggle
              ...
              aria-label={intl.formatMessage({id: 'components.TripNotificationsPane.toggleAdvancedSettings'})}
                  ...
                  <FormattedMessage
                    ...
                    values={{
                      minutes: (
                        ...
                        >
                          <DurationOptions defaultValue={30} minuteOptions={[15, 30, 45, 60]} />
                        </Select>
                      )
                    }}
                  />
...
export default connect()(injectIntl(TripNotificationsPane))
