...
export default function upcomingTripRenderer ({
  ...
  data.bodyText =
    `Trip is due to begin at ${tripStartMoment.format(timeFormat)}. (
      realtime monitoring will begin at ${monitoringStartMoment.format(timeFormat)}
    )`
  ...
  if (minutesUntilTripStart > monitoredTrip.leadTimeInMinutes) {
    ...
    if (data.journeyState.hasRealtimeData) {
      ...
      if (Math.abs(departureDeviationSeconds) < onTimeThresholdDeltaSeconds) {
        // about on time
