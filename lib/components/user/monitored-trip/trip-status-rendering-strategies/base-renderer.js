...
export default function baseRenderer (monitoredTrip) {
  const data = {
    ...
    body: <FormattedMessage id='components.BaseRenderer.bodyDefault' />,
    headingText: <FormattedMessage id='components.BaseRenderer.bodyDefault' />, // same default msg as body
    journeyState: monitoredTrip && monitoredTrip.journeyState,
    lastCheckedText: <FormattedMessage id='components.BaseRenderer.lastCheckedDefaultText' />,
  ...
  if (data.tripIsActive) {
    ...
    data.togglePauseTripButtonText = <FormattedMessage
      id='components.BaseRenderer.togglePause'
    />
  } else {
    data.togglePauseTripButtonGlyphIcon = 'play'
    data.togglePauseTripButtonText = <FormattedMessage
      id='components.BaseRenderer.untogglePause'
    />
  }
  ...
  if (data.tripIsSnoozed) {
    ...
    data.toggleSnoozeTripButtonText = <FormattedMessage
      id='components.BaseRenderer.tripIsSnoozed'
    />
  } else {
    data.toggleSnoozeTripButtonGlyphIcon = 'pause'
    data.toggleSnoozeTripButtonText = <FormattedMessage
      id='components.BaseRenderer.tripIsNotSnoozed'
    />
  }
