...
export default function baseRenderer (monitoredTrip) {
  const data = {
    ...
    body: <FormattedMessage id='components.TripStatusRenderers.base.unknownState' />,
    headingText: <FormattedMessage id='components.TripStatusRenderers.base.unknownState' />, // same default msg as body
    journeyState: monitoredTrip && monitoredTrip.journeyState,
    lastCheckedText: <FormattedMessage id='components.TripStatusRenderers.base.lastCheckedDefaultText' />,
  ...
  if (data.journeyState) {
    ...
    data.lastCheckedText = <FormattedMessage
  ...
  if (data.tripIsActive) {
    ...
    data.togglePauseTripButtonText = <FormattedMessage
      id='components.TripStatusRenderers.base.togglePause'
    />
  } else {
    data.togglePauseTripButtonGlyphIcon = 'play'
    data.togglePauseTripButtonText = <FormattedMessage
      id='components.TripStatusRenderers.base.untogglePause'
    />
  }

  if (data.tripIsSnoozed) {
    data.toggleSnoozeTripButtonGlyphIcon = 'play'
    data.toggleSnoozeTripButtonText = <FormattedMessage
      id='components.TripStatusRenderers.base.tripIsSnoozed'
    />
  } else {
    data.toggleSnoozeTripButtonGlyphIcon = 'pause'
    data.toggleSnoozeTripButtonText = <FormattedMessage
      id='components.TripStatusRenderers.base.tripIsNotSnoozed'
    />
  }
