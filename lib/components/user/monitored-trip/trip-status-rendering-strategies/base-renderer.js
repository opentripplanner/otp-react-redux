...
export default function baseRenderer(monitoredTrip) {
  ...
  // Set some alert data, with the corresponding agency name, if the matching itinerary exists.
  data.alerts = []
  data.matchingItinerary?.legs.forEach((leg) => {
    leg.alerts?.forEach((alert) => {
      data.alerts.push({
        ...alert,
