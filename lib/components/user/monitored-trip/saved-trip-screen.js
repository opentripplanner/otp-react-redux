...
class SavedTripScreen extends Component {
  ...
  _updateMonitoredTrip = async (monitoredTrip) => {
    ...
    await createOrUpdateUserMonitoredTrip(
      tripToSave,
      isCreating,
      undefined,
      undefined,
    ...
    // After saving, let this trip be the new undirty reference,
    // so that we don't prompt for navigation after saving.
    this.setState({ createdTrip: tripToSave })
  }
  ...
  _panes = {
    ...
    readOnlyAlert: TripReadonlyPane,
  ...
  componentDidMount() {
    ...
    // Reset page to top when opening a saved trip
    window.scrollTo(0, 0)
    if (isCreating) {
  ...
  _getTripToEdit = () => {
    ...
    return isCreating
      ? this.state.createdTrip || this._createMonitoredTrip()
...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    ...
    disableSingleItineraryDays,
    hasMobilityProfile: isModuleEnabled(state, Modules.MOBILITY_PROFILE),
