...
class SavedTripScreen extends Component {
  ...
  render () {
    ...
    if (!monitoredTrips) {
      ...
      // Create a modified copy of monitoredTrip for editing.
      const clonedMonitoredTrip = clone(monitoredTrip)

      // To spare users the complexity of the departure/arrival delay thresholds,
      // we create an overall delayThreshold field for UI editing, initialized as the smallest
      ...
      screenContents = (
        <Formik
          ...
          enableReinitialize
          initialValues={clonedMonitoredTrip}
