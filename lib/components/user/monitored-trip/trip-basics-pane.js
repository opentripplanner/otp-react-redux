...
class TripBasicsPane extends Component {
  ...
  render () {
    ...
    const unsavedChangesMessage = intl.formatMessage({
      id: `components.TripBasicsPane.unsavedChanges${isCreating ? 'New' : 'Existing'}Trip`
    })
      ...
      return (
        <div>
          ...
          <FormGroup validationState={errorStates.tripName}>
            ...
            {errors.tripName && (
              <HelpBlock>
                <FormattedMessage id={errors.tripName} />
              </HelpBlock>
            )}
          </FormGroup>
              ...
              {ALL_DAYS.map(day => {
                ...
                const notAvailableText = isDayDisabled
                  ? intl.formatMessage(
                    ...
                    {repeatedDay: intl.formatMessage({id: `common.daysOfWeekPlural.${day}`})}
                  )
                  : null

                return (
                  <TripDayLabel className={boxClass} key={day} title={notAvailableText}>
                    <span>
                      <FormattedMessage id={`common.daysOfWeekCompact.${day}`} />
                    </span>
            ...
            <HelpBlock>
              {itineraryExistence
                ? (
                  ...
                  <ProgressBar
                    active
                    label={<FormattedMessage id='components.TripBasicsPane.checkingItineraryExistence' />}
                    now={100}
                  />
                )
              }
