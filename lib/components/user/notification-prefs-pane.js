...
class NotificationPrefsPane extends Component {
  _handleOnPhoneUpdated = ({ isPhoneNumberVerified, phoneNumber }) => {
    const { setFieldValue } = this.props

    // Update the parent's Formik state, so that the updated numbers are rendered,
    // even when changing the notification channel back and forth.
...
class PhoneNumberEditor extends Component {
  ...
  _handleRequestCode = async () => {
    ...
    const { onCodeRequested, onRequestCode, setFieldValue, setFieldTouched } = this.props
    ...
    if (newPhoneNumber === currentNumber && currentNumberVerified) {
      ...
      const { isPhoneNumberVerified, phoneNumber } = result
        ...
        {isEditing
          ? (
            ...
            <FormGroup>
              ...
              <ControlStrip>
                <InlineStatic>
                  {formatPhoneNumber(isPending ? newPhoneNumber : currentNumber)}
                  {' '}
                  {isPending || !currentNumberVerified
