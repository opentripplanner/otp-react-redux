...
class NavLoginButton extends Component {
  ...
  constructor (props) {
    ...
    // Copy the link items and add an onClick handler to each copy.
    if (props.links) {
      this.links = props.links.map(({ target, text, url }) => ({
        onClick: () => props.routeTo(url),
        target,
        text
      }))
    }
    ...
    if (profile) {
      ...
      return (
        <NavDropdown
          ...
          {this.links && this.links.map(({ onClick, target, text }, index) => (
            <MenuItem
              onSelect={onClick}
              key={index}
              target={target}>
