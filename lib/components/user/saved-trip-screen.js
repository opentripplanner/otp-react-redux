...
class SavedTripScreen extends Component {
  ...
  constructor (props) {
    ...
    const { itineraries, loggedInUser, queryParams, wizard } = props
    const thisMonitoredTrip = wizard ? getNewMonitoredTrip(loggedInUser, queryParams, itineraries) : null
  ...
  _fetchTrips = async () => {
    const { auth, persistence, routeTo, wizard } = this.props
  ...
  _handleDeleteTrip = async () => {
    if (confirm('Would you like to remove this trip?')) {
      ...
      const deleteResult = await deleteTrip(persistence.otp_middleware, auth.accessToken, monitoredTrip)
