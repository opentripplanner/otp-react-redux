...
class SavedTripScreen extends Component {
  ...
  async componentDidMount () {
    const { ensureUserMonitoredTripsAreLoaded, isCreating, monitoredTrips } = this.props
    if (!monitoredTrips) {
      // If user monitored trips are not loaded, ensure they are
      // before checking the limit on saved trips.
      await ensureUserMonitoredTripsAreLoaded()
    }
  ...
  render () {
    ...
    let screenContents
    if (!monitoredTrips) {
      // Flash an indication while user trips are being loaded the first time.
...
const mapDispatchToProps = {
  ...
  ensureUserMonitoredTripsAreLoaded: userActions.ensureUserMonitoredTripsAreLoaded,
