...
class SavedTripScreen extends Component {
  ...
  _handleSaveNewTrip = async () => {
    ...
    // For new trips, if a trip with the same ui_activeSearch value in queryParams
    // appears in the monitored trips redux state,
    // then we know that save was successful and we can proceed.
    const { activeSearchId } = this.props
  ...
  _handleSaveTripEdits = async () => {
    ...
    // For existing trips, if the lastUpdated value (long) for the trip with the same id in monitored trips redux state
    // is more recent that our copy, then we know that save was successful and we can proceed.
    // (The state is not updated upon save because the next normal step is to close this screen.)
    const savedTrip = this.props.monitoredTrips.find(trip => trip.id === monitoredTrip.id)

    if (savedTrip.lastUpdated > monitoredTrip.lastUpdated) {
      this._goToSavedTrips()
    }
  }
