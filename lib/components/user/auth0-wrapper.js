...
const Auth0Wrapper = props => {
  const { children, awaitToken, persistence, showAwaitingToken } = props
  if (getAuth0Config(persistence)) {
    ...
    if (awaitToken) {
      return (
        <Auth0TokenWrapper
          auth={auth}
          // isAuthenticated is set explicitly so we can trigger a render/mount when that prop changes.
          isAuthenticated={auth.isAuthenticated}
          showAwaitingToken={showAwaitingToken}
        >
          {childrenWithAuth}
        </Auth0TokenWrapper>
      )
