 ...
 * (The Auth0TokenWrapper component handles getting/forwarding the token.)
 *
 * If Auth0 is disabled, the component just renders the children.
 *
 ...
 */
const Auth0Wrapper = ({ children, awaitToken, persistence, showAwaitingScreen }) => {
  if (getAuth0Config(persistence)) {
    ...
    if (awaitToken) {
      return (
        // Inlining Auth0TokenWrapper doesn't seem to work
        // (if an accessToken becomes available, Auth0Wrapper is not rendered again).
        <Auth0TokenWrapper
          auth={auth}
          showAwaitingScreen={showAwaitingScreen}
        >
          {childrenWithAuth}
        </Auth0TokenWrapper>
      )
...
Auth0Wrapper.propTypes = {
   ...
   * before displaying children (some children may require a token).
   * Does not apply if no user is logged in.
   */
  awaitToken: PropTypes.bool,

  /** Determines whether a screen is shown while a token is being retrieved. */
  showAwaitingScreen: PropTypes.bool
}
