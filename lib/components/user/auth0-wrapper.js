...
const Auth0Wrapper = props => {
  const { children, /* auth0Props, */ awaitToken, persistence } = props
  const auth = useAuth({
    audience: persistence.auth0.audience,
    scope: AUTH0_SCOPE
  })
  //  const auth0PropObject = {}
  //  for (let prop in auth0Props) {
  //    auth0PropObject[prop] = auth0Props[prop]
  //  }

  //  const childrenWithAuth = renderChildrenWithProps(children, { auth, auth0Props })
...
export default withLoginRequired(connect(mapStateToProps, mapDispatchToProps)(Auth0Wrapper))
