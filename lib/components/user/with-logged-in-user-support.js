...
class UserLoaderScreen extends Component {
  async componentDidUpdate () {
    const { auth0, fetchOrInitializeUser, loggedInUser } = this.props

    if (auth0 && auth0.isAuthenticated && !loggedInUser) {
      try {
        // Get the token
        const token = await auth0.getAccessTokenSilently()
        // Once accessToken is available, proceed to fetch or initialize loggedInUser.
        fetchOrInitializeUser(auth0, token)
      } catch (error) {
  ...
  render () {
    const { auth0, children, loggedInUser, requireLoggedInUser } = this.props

    if (auth0) {
      if (requireLoggedInUser && auth0.isAuthenticated && !loggedInUser) {
