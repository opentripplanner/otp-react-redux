...
const validationSchemaShape = {
  address: yup.string().required('components.FavoritePlaceScreen.invalidAddress'),
  name: yup.string().required('components.FavoritePlaceScreen.invalidName')
}
...
class FavoritePlaceScreen extends Component {
  ...
  _getFullValidationSchema = (places, place) => {
    ...
    clonedSchemaShape.name = yup.string()
      .required('components.FavoritePlaceScreen.invalidName')
      .notOneOf(otherPlaceNames, 'components.FavoritePlaceScreen.nameAlreadyUsed')
