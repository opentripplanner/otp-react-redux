...
class PlaceShortcut extends Component {
  ...
  render () {
    ...
    const convertedPlace = path ? place : convertToPlace(place, true)
    const { address, icon, name, type } = convertedPlace
    ...
    if (path) { // for middleware places
      ...
      placeTitle = `${placeName}${details && ` (${details})`}`
    } else {
      placeName = formatStoredPlaceName(place, false)
      placeTitle = formatStoredPlaceName(place)
