...
/**
 * If the browser timezone is not the homeTimezone, appends a time zone designation (e.g., PDT)
 * to designate times in the homeTimezone
 * (e.g., user in New York, but viewing a trip planner for service based in Los Angeles).
 */
function getTimeFormat (timeFormat, homeTimezone) {
  const userTimeZone = getUserTimezone()
  const inHomeTimezone = homeTimezone && homeTimezone === userTimeZone
  return inHomeTimezone ? timeFormat : `${timeFormat} z`
}
...
const StopTimeCell = ({
  ...
  const formattedTime = formatSecondsAfterMidnight(
    ...
    getTimeFormat(timeFormat, homeTimezone)
  )
