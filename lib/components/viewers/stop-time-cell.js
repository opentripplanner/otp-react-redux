...
const StopTimeCell = ({
  ...
  // Convert seconds after midnight to unix (millisceonds) for FormattedTime
  var current = new Date()
  var startOfDay = new Date(current.getFullYear(), current.getMonth(), current.getDate())
      ...
      <div style={{ fontSize: showDayOfWeek ? 12 : 14, marginLeft: 20 }}>
        ...
        <div>
          {showCountdown
            ...
            : <FormattedTime
              timeFormat='short'
              value={startOfDay.getTime() + departureTime * 1000}
            />
          }
        </div>
      </div>
