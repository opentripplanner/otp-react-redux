...
const StopTimeCell = ({
  ...
  if (!homeTimezone || !stopTime) {
    console.warn('Missing required prop(s) for StopTimeCell: homeTimezone, stopTime')
    return <div><FormattedMessage id='common.forms.error' /></div>
  }
  ...
  // Convert seconds after miSdnight to unix (milliseconds) for FormattedTime
  console.log('serviceDay value', stopTime.serviceDay * 1000)
  var startOfDay = moment(stopTime.serviceDay * 1000).startOf('day').valueOf()
  console.log('Start of day log:', startOfDay)
  var departureTimestamp = startOfDay + departureTime * 1000
  console.log('Departure timestamp log:', departureTimestamp)
