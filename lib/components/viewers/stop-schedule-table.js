      ...
      <StyledTable>
        ...
        <tbody>
          {mergedStopTimes.map((stopTime, index) => {
            ...
            // Generate epoch time for departure
            const departureUnixTimestamp = moment(stopTime.serviceDay * 1000).tz(homeTimezone).valueOf() + stopTime.scheduledDeparture * 1000
            ...
            return (
              <tr className={className} key={index} ref={refProp}>
                ...
                <td>{headsign}</td>
                <TimeCell>
                  <FormattedTime timeFormat='short' timeZone={homeTimezone} value={departureUnixTimestamp} />
                </TimeCell>
              </tr>
            )
          })}
