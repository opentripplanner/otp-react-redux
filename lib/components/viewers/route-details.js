...
class RouteDetails extends Component {
  ...
  refreshVehiclePositions = () => {
    ...
    if (route?.id) getVehiclePositionsForRoute(route.id)
  }

  componentDidUpdate = (prevProps) => {
    const { route, vehiclePositionRefreshSeconds } = this.props
    if (!prevProps.route?.id && route?.id) {
      // Fetch vehicle positions when initially showing component
      // (and when route info has been fetched).
