...
class RouteDetails extends Component {
  ...
  componentDidMount = () => {
    ...
    // Fetch vehicle positions when loading for the first time.
    if (!route.vehicles) {
      this.refreshVehiclePositions()
    }
    // Refresh vehicle positions every 30s. TODO: config.
    this.setState({
      refreshTimer: setInterval(this.refreshVehiclePositions, 30000)
    })
  }
