...
class TripViewer extends Component {
  static propTypes = {
    ...
    setActiveItinerary: narrativeActions.setActiveItinerary.type,
  ...
  componentDidUpdate(prevProps) {
    ...
    if (!this.props.viewedTrip.fromStopId || !this.props.viewedTrip.toStopId) {
      if (!this.props.activeItinerary) {
        this.props.setActiveItinerary({
          index: this.props.activeItineraryIndex
        })
        // this.props.setVisibleItinerary({ index: this.props.activeItineraryIndex })
      } else {
        const matchingLeg = this.props.activeItinerary?.legs
          ?.filter(coreUtils.itinerary.isTransitLeg)
          .find((leg) => leg.tripId === this.props.viewedTrip?.tripId)
        this.props.setViewedTrip({
          fromStopId: matchingLeg.from?.stopId,
          toStopId: matchingLeg.to?.stopId,
          tripId: this.props.viewedTrip.tripId
        })
      }
    } else if (!this.props.activeItinerary && prevProps.activeItinerary) {
...
const mapDispatchToProps = {
  ...
  setActiveItinerary: narrativeActions.settingActiveItinerary,
