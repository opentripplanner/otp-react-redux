...
class TripViewer extends Component {
  ...
  getStopIndexFromStopId = (stopId, minIndex = -1) => {
    // TODO: use minIndex
    const { tripData } = this.props
    return tripData.stops.findIndex(
      (stop, index) => index > minIndex && stop.gtfsId === stopId
    )
  }
  ...
  render() {
    ...
    if (viewedTrip.fromStopId) {
      viewedTrip.fromIndex = this.getStopIndexFromStopId(viewedTrip.fromStopId)
    }
    if (viewedTrip.toStopId) {
      viewedTrip.toIndex = this.getStopIndexFromStopId(viewedTrip.toStopId)
    }
