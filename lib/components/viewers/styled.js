...
export const StyledRouteViewerContainer = styled.div`
  background-color: ${(props) => props.routeColor || '#fff'};
  color: ${(props) => props.textColor || 'inherit'};
  fill: ${(props) => props.textColor || 'inherit'};
  height: 100%;
  // TODO: transition back when pattern viewer back button is enabled
  transition: ${(props) =>
    props.patternView && 'background-color ease-in 250ms'};

  & > * {
    opacity: ${(props) => (props.fade ? '0%' : '100%')};
    transition: opacity ease-out 100ms;
    transition-delay: ${(props) => (props.patternView ? '150ms' : '0ms')};
