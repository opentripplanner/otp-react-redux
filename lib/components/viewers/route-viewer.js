...
function operatorForRoute (transitOperators, route) {
  if (!route.id) return
  const index = transitOperators.findIndex(o =>
    o.id.toLowerCase() === route.id.split(':')[0].toLowerCase())
  if (index !== -1 && typeof transitOperators[index].order !== 'undefined') return transitOperators[index]
}

function routeOperatorComparatorValue (transitOperators, route) {
  const routeOperator = operatorForRoute(transitOperators, route)
