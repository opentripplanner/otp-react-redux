...
import { prefersReducedMotion } from '../../util/preferReducedMotion'
import { StyledIconWrapper } from '../util/styledIcon'
import PageTitle from '../util/page-title'

import { RouteName, RouteRow } from './RouteRow'
import { StyledRouteViewerContainer } from './styled'
...
class RouteViewer extends Component {
   ...
   * - if a pattern is selected, add the pattern destination too.
   */
  getTitle = () => {
  ...
  componentDidUpdate() {
    const { routes, viewedRoute, viewedRouteObject } = this.props
    const { fade, initialRender } = this.state

    if (viewedRoute?.patternId && viewedRouteObject && fade) {
      setTimeout(() => this.setState({ fade: false }), 15)
    }
  ...
  onPatternButtonClick = (firstPattern, routeId) => {
    const { setViewedRoute } = this.props
    this.setState({ fade: true })
    prefersReducedMotion
      ? setViewedRoute({ patternId: firstPattern, routeId })
      : setTimeout(
          () => setViewedRoute({ patternId: firstPattern, routeId }),
          100
        )
  ...
  render() {
    ...
    if (viewedRoute?.patternId && viewedRouteObject) {
      ...
      return (
        <StyledRouteViewerContainer
          className="route-viewer"
          fade={!prefersReducedMotion && fade}
          patternView
          ...
          {sortedRoutes.map((route) => {
            ...
            return (
              <RouteRow
                ...
                key={route.id}
                onPatternButtonClick={this.onPatternButtonClick}
                operator={operator}
                route={route}
