...
import RouteDetails, { getOperatorName } from './route-details'
...
class RouteViewer extends Component {
  ...
  getRouteOperator = () => {
    const { transitOperators, viewedRouteObject } = this.props
    return (
  ...
  /**
   * Gets the title following these rules:
   * - if no route is selected, say "Route Viewer",
   * - if a route is selected, display the agency and user-facing route number
   * - if a pattern is selected, add the pattern destination.
   */
  getTitle = () => {
    const { intl, viewedRoute, viewedRouteObject } = this.props
    const { patternId } = viewedRoute || {}
    const { agency, patterns, pending, shortName } = viewedRouteObject || {}
    ...
    const operatorName = getOperatorName(this.getRouteOperator(), agency)

    // Build breadcrumb-like, user text with user-friendly values (operator route direction),
  ...
  render() {
    ...
    if (viewedRoute?.patternId && viewedRouteObject) {
      ...
      // Find operator based on agency_id (extracted from OTP route ID).
      const operator = this.getRouteOperator()
