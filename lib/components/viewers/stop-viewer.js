...
class StopViewer extends Component {
  ...
  componentDidMount() {
    ...
    console.log('stop viewer mount', map, viewedStop)
  ...
  componentDidUpdate(prevProps, prevState) {
    const { fetchStopInfo, homeTimezone, map, viewedStop, zoomToStop } =
      this.props
    const { date, isShowingSchedule } = this.state
    ...
    if (prevProps.viewedStop?.stopId !== viewedStop?.stopId && map) {
      ...
      fetchStopInfo(map, viewedStop)
    } else if (map && prevProps.map !== map) {
      // If only the map instance was updated, then just zoom to the stop.
      zoomToStop(map, viewedStop)
  ...
  _zoomToStop = () => {
    const { map, stopData, zoomToStop } = this.props
    zoomToStop(map, stopData)
  }
