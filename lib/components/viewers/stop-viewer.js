 ...
 */
function queryParams(addedParams) {
  const search = {
      ...
      >
        <div>
          ...
          <a
            ...
            // TODO: Extract function to build nearby view URL.
            href={`/#/nearby/${stopData.lat},${stopData.lon}?${queryParams({
              entityId: stopData.id
            })}`}
          ...
          <Link
            ...
            style={{ color: 'inherit', fontSize: 'small' }}
            to={{
              pathname: `/nearby/${stopData.lat},${stopData.lon}`,
              search: queryParams({ entityId: stopData.id })
            }}
          >
            l:
            {/* FIXME: What icon should we use? */}
