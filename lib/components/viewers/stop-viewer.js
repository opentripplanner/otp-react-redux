 ...
 */
function getHomeSecondsAfterMidnight (homeTimezone) {
  const now = moment()
...
function getFormattedStopTime (stopTime, homeTimezone, soonText = 'Due', timeFormat) {
  ...
  const arrivalDay = serviceDay.add(daysAfterServiceDay, 'day')
  // Calculate current home seconds after midnight.
  const currentHomeSecondsAfterMidnight = getHomeSecondsAfterMidnight(homeTimezone)
  console.log(stopTime, currentHomeSecondsAfterMidnight)
  ...
  // time. If arrival occurs on different (AKA future) day, use unaltered stop
  // time departure. If the arrival occurs on the same day, use remainder
  // departure time so that we're comparing times based on actual seconds since
  // midnight (without interference from times that occur greater than 24 hours
  // after start of service).
  const secondsUntilDeparture = differentDay
    ? stopTime.realtimeDeparture - currentHomeSecondsAfterMidnight
    : departureTimeRemainder - currentHomeSecondsAfterMidnight
