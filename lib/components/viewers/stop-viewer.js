...
class StopViewer extends Component {
  ...
  _renderHeader = () => {
    const { enableFavoriteStops, hideBackButton, showUserSettings, stopData } = this.props
    return (
      ...
      <div style={{ marginBottom: '10px' }}>
        <div>
          ...
          <button
            ...
            title={intl.formatMessage({id: 'components.StopViewer.zoomToStop'})}
          >
            <Icon type="search" />
          </button>
          {!isFlex && <button
  ...
  render() {
    ...
    if (!hasStopTimesAndRoutes) {
      ...
      if (stopIsFlex(stopData)) {
        contents = <div style={{lineHeight:'normal'}}><FormattedMessage id="components.StopViewer.flexStop" /></div>
      }
...
const mapStateToProps = (state, ownProps) => {
  ...
  const { homeTimezone, language, persistence, stopViewer, transitOperators } = config
  ...
  return {
    autoRefreshStopTimes,
    enableFavoriteStops: getPersistenceStrategy(persistence) === PERSIST_TO_LOCAL_STORAGE,
    favoriteStops,
    homeTimezone,
    nearbyStops,
...
export default connect(mapStateToProps, mapDispatchToProps)(injectIntl(StopViewer))
