...
const dateFormat = 'MM/DD/YYYY' // TODO: Make configurable
...
class DateTimeSelector extends Component {
  ...
  _onBackupDateChange = (evt) => {
    const date = moment(evt.target.value, dateFormat).format('YYYY-MM-DD')
  ...
  render () {
    const { departArrive, date, time, startTime, endTime, timeFormat } = this.props
