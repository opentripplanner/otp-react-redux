  ...
  render () {
    ...
    return (
      <div className={`user-settings ${className}`}>
        ...
        <Places
          getDetailText={location => location.address || (!isHomeOrWork(location) && location.name) || `Set your ${location.type} address`}
          getMainText={location => isHomeOrWork(location) ? location.type : location.name}
