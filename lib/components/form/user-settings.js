...
function summarizeQuery(query, intl, locations = []) {
  const from =
...
/**
 * Formats ellapsed time from the specified timestamp to now,
 * as in "5 hours ago" or the localized equivalent.
 */
function formatEllapsedTime(timestamp, intl) {
    ...
    return (
      <>
        ...
        {storeTripHistory && (
          <Places
            getDetailText={(location) => {
              return formatEllapsedTime(location.timestamp, intl)
            }}
            getMainText={(location) => location.address || location.name}
            header={
  ...
  render() {
    ...
    return (
      <div className={`user-settings ${className}`} style={style}>
        ...
        <Places
          getDetailText={(location) => {
            return getPlaceDetail(location, intl, isUsingOtpMiddleware)
          }}
          getMainText={(location) => {
            return getPlaceMainText(location, intl, isUsingOtpMiddleware)
          }}
          header={
...
const TripRequest = ({ forgetSearch, setQueryParam, tripRequest, user }) => {
  ...
  return (
    <StyledMainPanelPlace
      ...
      detailText={formatEllapsedTime(timestamp, intl)}
      icon="clock-o"
      mainText={mainText}
      onClick={_onSelect}
...
const mapStateToProps = (state) => {
  ...
  return {
    ...
    loggedInUserTripRequests,
    nearbyStops,
    persistenceStrategy: getPersistenceStrategy(config.persistence),
    query: currentQuery,
    sessionSearches,
