...
class UserSettings extends Component {
  ...
  _getLocations = () => {
    ...
    const locations = clone(user.locations)
    ...
    // Add default 'home' and 'work' placeholders if the corresponding entry is neither in user or loggedInUser.
    if (!locations.find(isWork) &&
        !savedLocations.find(isWork)) {
      locations.push({
        id: 'work',
        type: 'work',
        icon: 'briefcase',
  ...
  render () {
    ...
    // TODO: insert additional location types before 'custom'.
    const order = ['home', 'work', 'dining', 'custom', 'suggested', 'stop', 'recent']
...
class Place extends Component {
  ...
  _isForgettable = () => {
    ...
    // For simplicity, locations from the loggedInUser's account page are not forgettable from this pane.
    return location.origin !== 'loggedInUser' &&
      ['stop', 'home', 'work', 'recent'].indexOf(location.type) !== -1
  }
  ...
  render () {
    ...
    // Render the 'address' field if the location type is not supported by getDetailText
    // and if it is different than the location's name.
    if (!detailText && location.address !== location.name) {
