...
import CachedPlace from '../user/places/cached-place'
import SavedPlace from '../user/places/saved-place'
import PlaceShortcut from '../user/places/place-shortcut'
import { MainPanelPlace } from '../user/places/styled'
...
/**
 * Associate a styled Place component to the provided Place component for rendering.
 */
function stylePlace (BasePlaceComponent) {
  return props => (
    <BasePlaceComponent
      PlaceComponent={MainPanelPlace}
      {...props}
    />
  )
}

const StyledCachedPlace = stylePlace(CachedPlace)
const StyledSavedPlace = stylePlace(SavedPlace)
...
class UserSettings extends Component {
  ...
  render () {
    ...
    if (loggedInUser && persistenceStrategy === PERSIST_TO_OTP_MIDDLEWARE) {
      return (
        <div className={`user-settings ${className}`}>
          <Places
            ...
            PlaceComponent={StyledSavedPlace}
            places={loggedInUser.savedLocations}
            separator={false}
          />
          <RecentTrips
      ...
      const { favoriteStops, storeTripHistory, recentPlaces, recentSearches } = localUser
...
const Places = ({
  ...
  onDelete,
  onView,
  PlaceComponent = StyledCachedPlace,
  places,
  separator = true,
  textIfEmpty
}) => {
