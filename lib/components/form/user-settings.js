...
const TripRequest = ({
  ...
  const _onSelect = useCallback(
    ...
    () => {
      ...
      // Also, pass query params without the mode, otherwise the selection from the mode selector will not apply.
      const { modes, ...queryWithoutModes } = query
      // If depart/arrive is 'NOW', change that to 'DEPART' so that the correct time is shown in the date button.
