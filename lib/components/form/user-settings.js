...
import { getPersistenceStrategy, isHome, isHomeOrWork, isWork } from '../../util/user'
...
class UserSettings extends Component {
    ...
    return (
      <>
        ...
        <Places
          ...
          header={<FormattedMessage id='components.UserSettings.favoriteStops' />}
          onDelete={forgetStop}
          onView={setViewedStop}
          places={favoriteStops}
          textIfEmpty={<FormattedMessage id='components.UserSettings.noFavoriteStops' />}
        />

        {storeTripHistory && <Places
          getDetailText={location => moment(location.timestamp).fromNow()}
          getMainText={location => location.address || location.name}
          header={<FormattedMessage id='components.UserSettings.recentPlaces' />}
          onDelete={deleteLocalUserRecentPlace}
          places={recentPlaces}
        />}
        {storeTripHistory && <RecentTrips
  ...
  render () {
    ...
    // app state/store.
    const locations = this._getLocations(isUsingOtpMiddleware ? loggedInUser : localUser)
      ...
      <div className={`user-settings ${className}`} style={style}>
        ...
        <Places
          getDetailText={location =>
            ...
            (isUsingOtpMiddleware ? location.address : location.name) || `Set your ${location.type} address`}
          getMainText={location => isHomeOrWork(location) ? toSentenceCase(location.type) : location.name}
...
class TripRequest extends Component {
  ...
  _onForget = () => this.props.forgetSearch(this.props.tripRequest)

  render () {
    const { intl, tripRequest, user } = this.props
    ...
    return (
      <StyledMainPanelPlace
        ...
        onDelete={canDelete ? this._onForget : null}
      />
    )
  }
}
