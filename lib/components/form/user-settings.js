...
function formatElapsedTime(timestamp, intl) {
  ...
  if (days === 0) {
    if (hours === 0) {
      if (minutes === 0) {
        return intl.formatMessage({ id: 'common.time.duration.aFewSeconds' })
      }
    ...
    return intl.formatMessage({ id: 'common.time.duration.nHours' }, { hours })
  }
  return intl.formatMessage({ id: 'common.time.duration.nDays' }, { days })
}
