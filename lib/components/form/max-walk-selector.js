...
import { setMaxWalkDistance } from '../../actions/form'

class MaxWalkSelector extends Component {
  static propTypes = {
    maxWalkDistance: PropTypes.number,
    setMaxWalkDistance: PropTypes.func
  }
  ...
  render () {
    ...
    console.log('mWD', maxWalkDistance);
    return (
      <Form>
          ...
          <Row>
            ...
            <Col xs={6}>
              <FormControl
                ...
                onChange={(evt) => {
                  this.props.setMaxWalkDistance(evt.target.value)
                }}
              >
                {options.map((o, i) => (
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    setMaxWalkDistance: (maxWalkDistance) => { dispatch(setMaxWalkDistance({ maxWalkDistance })) }
  }
}
