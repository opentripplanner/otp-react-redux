...
class LocationField extends Component {
  ...
  render () {
    ...
    if (geocodedFeatures.length > 0) {
      ...
      // menuItems.push(<DropdownItem header key='sr-header'>Search Results</DropdownItem>)
    ...
    if (nearbyStops.length > 0 && !suppressNearby) {
      // Add the menu sub-heading (not a selectable item)
      menuItems.push(<DropdownItem header key='ns-header'>Nearby Stops</DropdownItem>)
    ...
    if (sessionSearches.length > 0) {
      // Add the menu sub-heading (not a selectable item)
      menuItems.push(<DropdownItem header key='ss-header'>Recently Searched</DropdownItem>)
    ...
    if (locations.length > 0 && showUserSettings) {
      // Add the menu sub-heading (not a selectable item)
      menuItems.push(<DropdownItem header key='mp-header'>My Places</DropdownItem>)
...
function createTransitStopOption (stop, onSelect, isActive) {
  return <DropdownItem className='location-option' onSelect={onSelect} key={itemKey++} active={isActive}>
    <div>
