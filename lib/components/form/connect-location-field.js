...
import { isWork } from '../../util/user'

// Custom user location icon to be consistent with the account page.
const UserLocationIcon = ({ userLocation }) => {
  const { icon = 'marker' } = userLocation
  // localStorage locations that are assigned the 'work' icon
...
export default function connectLocationField (StyledLocationField, options = {}) {
  ...
  const mapStateToProps = (state, ownProps) => {
    ...
    let loggedInUserLocations = loggedInUser ? clone(loggedInUser.savedLocations) : []
    loggedInUserLocations = loggedInUserLocations.filter(loc => !isBlank(loc.address))
    loggedInUserLocations.filter(isWork).forEach(loc => { loc.icon = 'work' })
    loggedInUserLocations.forEach(loc => { loc.name = loc.address })
