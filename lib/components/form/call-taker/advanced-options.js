...
/**
 * Converts a set of planConnection modes to a list of mode strings
 */
const convertPlanConnectionModesToStringArray = (m) => {
  const modes = [
    ...(m?.direct || []),
    ...(m?.transit?.access || []),
    ...(m?.transit?.egress || []),
...
const convertStringToModeObject = (m) => {
  return m.split('_') ? { mode: m.split('_')[0] } : { mode: 'WALK' }
}
...
class AdvancedOptions extends Component {
  ...
  _setBannedRoutes = (options) => {
    const bannedRoutes = options ? options.map((o) => o.value) : null
    this.props.setQueryParam({ bannedRoutes })
  }

  // TODO MOVE TO REQUIRED
  _setPreferredRoutes = (options) => {
    const preferred = options
      ? { routes: options.map((o) => o.value).join(',') }
      : {}
    this.props.setQueryParam({ preferred })
  }

  _isBannedRouteOptionDisabled = (option) => {
    // Disable routes that are preferred already.
    const preferredRoutes = this.getRouteList('bannedRoutes')
    return (
  ...
  _isPreferredRouteOptionDisabled = (option) => {
    ...
    const bannedRoutes = this.getRouteList('preferred')
  ...
  _onSubModeChange = (changedMode) => {
    ...
    const allModes = convertPlanConnectionModesToStringArray(
      this.props.currentQuery?.modes
    )
      .map((m) => {
        if (m === 'TRANSIT') {
          return transitModes
    ...
    this.props.setQueryParam({
      transitModes: allModes.map(convertStringToModeObject) || { mode: 'WALK' }
    })
  }
  ...
  getRouteList = (key) => {
    ...
    if (this.state.routeOptions) {
      return this.state.routeOptions.filter(
        (o) => routesParam.indexOf(o.value) !== -1
      )
    } else {
      // If route list is not available, default labels to route IDs.
      return routesParam.map((id) => ({ label: id, value: id }))
    }
  }
  ...
  render() {
    ...
    const applicableModeSettings = this.props.modeSettings
      ?.filter((ms) =>
        convertPlanConnectionModesToStringArray(currentModes)
          ?.flat()
          ?.some((mode) => checkIfModeSettingApplies(ms, { mode: mode }))
      )
