...
class AdvancedOptions extends Component {
  ...
  render() {
    const { currentQuery, intl, modes, modeSettingValues, onKeyDown } =
      this.props
    ...
    const applicableModeSettings = this.props.modeSettings.filter((ms) =>
      ...
      <div>
        ...
        >
          <DropdownSelector
            ...
            value={
              bikeEnabled
                ? modeSettingValues.bikeReluctance
                : modeSettingValues.walkReluctance
            }
          />
        ...
        <div style={{ paddingTop: '8px' }}>
          {applicableModeSettings.map((ms, index) => (
            <ModeSettingRenderer
              key={index}
              onChange={this._setCustomModeSetting}
              setting={populateSettingWithValue(this.props.modeSettingValues)(
                ms
              )}
            />
          ))}
