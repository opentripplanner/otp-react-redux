...
export default class DateTimeOptions extends Component {
  ...
  render () {
    const intl = useIntl()
    const {departArrive, onKeyDown} = this.props
    ...
    return (
      <>
        ...
        <OverlayTrigger
          overlay={<Tooltip id='time-tooltip'>{intl.formatTime(dateTime)}</Tooltip>}
          placement='bottom'
          trigger={['focus', 'hover']}
        >
          <input
            ...
            value={timeInput ? intl.formatTime(timeInput) : intl.formatTime(dateTime)}
          />
        </OverlayTrigger>
        <input
          className='datetime-slim'
