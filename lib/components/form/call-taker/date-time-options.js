...
const { getCurrentDate, OTP_API_DATE_FORMAT_DATE_FNS, OTP_API_TIME_FORMAT } =
  coreUtils.time
...
function dateToQueryParams(date, timeZone) {
  return {
    date: format(date, OTP_API_DATE_FORMAT_DATE_FNS, { timeZone }),
...
class DateTimeOptions extends Component {
  ...
  _setDepartArrive = (evt) => {
    ...
    const { homeTimezone, setQueryParam } = this.props
    if (departArrive === 'NOW') {
      const now = new Date()
      // If setting to leave now, update date/time to "now" in the agency's time zone
      // and start auto refresh to keep form input in sync.
      this.setState({ timeInput: '' })
      setQueryParam({
        departArrive,
        ...dateToQueryParams(now, homeTimezone)
      })
      if (!this.state.timer) {
        this._startAutoRefresh()
      }
      ...
      // and, if timeInput was blank, set it to "now" in the agency's time zone.
      this._stopAutoRefresh()
      if (this.state.timeInput === '') {
        ...
        this.setState({ timeInput: nowFormatted })
      }
      setQueryParam({ departArrive })
    }
  }
    ...
    return (
      <>
        ...
        <input
          ...
          type="date"
          value={format(dateTime, OTP_API_DATE_FORMAT_DATE_FNS, {
            timeZone: homeTimezone
          })}
        />
      </>
