...
const departureOptions = [
  {
    // Default option.
    childMessageId: 'components.DateTimeOptions.now',
    value: 'NOW'
  },
  {
    childMessageId: 'components.DateTimeOptions.departAt',
    value: 'DEPART'
  },
  {
    childMessageId: 'components.DateTimeOptions.arriveBy',
    value: 'ARRIVE'
  }
]
...
class DateTimeOptions extends Component {
  ...
  parseInputAsTime = (timeInput, date = moment().startOf('day').format('YYYY-MM-DD')) => {
    ...
    return (
      <>
        <select
          ...
          value={departArrive}
        >
          {departureOptions.map(o => <option key={o.value}>{intl.formatMessage({id: o.childMessageId})}</option>)}
        </select>
        <OverlayTrigger
          overlay={<Tooltip id='time-tooltip'>{intl.formatTime(dateTime.unix())}</Tooltip>}
          placement='bottom'
          trigger={['focus', 'hover']}
        >
          <input
            ...
            value={timeInput
              ? intl.formatTime(
                this.parseInputAsTime(timeInput).unix()
              )
              : intl.formatTime(dateTime.unix())}
          />
        </OverlayTrigger>
        <input
          className='datetime-slim'
...
export default connect()(injectIntl(DateTimeOptions))
