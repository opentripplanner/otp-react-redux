...
class ModeDropdown extends Component {
  ...
  _getModeOptions = () => {
    const {intl, modes} = this.props
    return [
      { children: intl.formatMessage({id: 'common.modes.transit'}), mode: 'TRANSIT', value: 'TRANSIT' },
      ...modes.exclusiveModes.map(mode =>
        ({
          children: intl.formatMessage({id: 'components.ModeDropdown.exclusiveMode'}, {mode: toSentenceCase(mode)}),
          mode,
          value: `${mode}`
        })
      ),
...
export default connect()(injectIntl(ModeDropdown))
