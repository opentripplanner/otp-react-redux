...
class FieldTripDetails extends Component {
  constructor(props) {
    super(props)
    this.state = {
      // Changes every time a field trip travel date is persisted
      // (used to reset the field trip date input).
      travelDateUpdateCount: 0
    }
  }
  ...
  _setRequestDate = (evt) => {
    ...
    if (newDate !== '') {
      ...
      const convertedRequestDate = moment(newDate).format(dateFormat)
      setRequestDate(request, convertedRequestDate, intl)
      this.setState({
        travelDateUpdateCount: this.state.travelDateUpdateCount + 1
      })
    }
  }
  ...
  _renderHeader = () => {
    ...
    const { travelDateUpdateCount } = this.state
    const { id, schoolName, travelDate } = request
    const travelDateAsMoment = moment(travelDate)
    const travelDateFormatted = travelDateAsMoment.format('YYYY-MM-DD')
    return (
      <WindowHeader>
        ...
        <div style={{ marginLeft: '28px' }}>
          <small>
            ...
            <input
              ...
              // Allows the input to be reset when a field trip value gets edited/reloaded,
              // or a different trip (with a different date) is selected in the field trip list.
              key={`${travelDateFormatted}-${travelDateUpdateCount}`}
