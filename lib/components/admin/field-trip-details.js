...
class FieldTripDetails extends Component {
  ...
  _setRequestDate = (evt) => {
    ...
    if (newDateValue !== '') {
      ...
      const newDate = parse(newDateValue, 'yyyy-MM-dd', new Date())
  ...
  _renderHeader = () => {
    ...
    const travelDateObject = parseDate(travelDate)
    const travelDateFormatted = format(travelDateObject, 'yyyy-MM-dd')
    return (
      <WindowHeader>
        ...
        <div style={{ marginLeft: '28px' }}>
          <small>
            ...
            <input
              ...
              type="date"
            />
            ({formatDistanceToNow(travelDateObject, { addSuffix: true })})
          </small>
        </div>
      </WindowHeader>
    )
