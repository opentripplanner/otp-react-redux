...
class TripStatus extends Component {
  ...
  render() {
    const { intl, outbound, request, saveable } = this.props
    const {
    ...
    return (
      <Full>
        ...
        {outbound ? (
          <Para className="field-trip-time-edit">
            <EditableSection
              fields={[
                {
              ...
              onChange={(_, newParams) =>
                this.props.setArriveDestinationTime(
                  request,
                  newParams.arriveTime,
                  intl
                  // eslint-disable-next-line prettier/prettier
                )}
              request={{ arriveTime: this._formatTime(arriveDestinationTime) }}
            />
          </Para>
        ) : (
          //
          <>
            <Para className="field-trip-time-edit">
              <EditableSection
                fields={[
                  {
                    fieldName: 'leaveDestinationTime',
                    inputProps: { type: 'time' },
                    label: 'Leave at'
                  }
                ]}
                intl={this.props.intl}
                onChange={(_, newParams) =>
                  this.props.setLeaveDestinationTime(
                    request,
                    newParams.leaveDestinationTime,
                    intl
                    // eslint-disable-next-line prettier/prettier
                )}
                request={{
                  leaveDestinationTime: this._formatTime(leaveDestinationTime)
                }}
              />
              <EditableSection
                fields={[
                  {
                    fieldName: 'arriveSchoolTime',
                    inputProps: { type: 'time' },
                    label: 'due back at'
                  }
                ]}
                intl={this.props.intl}
                onChange={(_, newParams) =>
                  this.props.setArriveSchoolTime(
                    request,
                    newParams.arriveSchoolTime,
                    intl
                    // eslint-disable-next-line prettier/prettier
                )}
                request={{
                  arriveSchoolTime: this._formatTime(arriveSchoolTime)
                }}
              />
            </Para>
          </>
...
const mapDispatchToProps = {
  ...
  setArriveDestinationTime: fieldTripActions.setArriveDestinationTime,
  setArriveSchoolTime: fieldTripActions.setArriveSchoolTime,
  setLeaveDestinationTime: fieldTripActions.setLeaveDestinationTime,
