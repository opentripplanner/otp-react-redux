...
class BaseMap extends Component {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      // TODO: ensure that this is initialized properly for any layers that are default-visible
      overlayVisibility: {}
    }
  }
  ...
  _onOverlayAdd = (evt) => {
    let overlayVisibility = {...this.state.overlayVisibility}
  ...
  _updateBounds (oldProps, newProps) {
    ...
    } else if (newProps.itinerary && newProps.activeLeg !== oldProps.activeLeg) {
    ...
    // Pan to default map center
    } else {
      map.leafletElement.panTo([config.map.initLat, config.map.initLon])
    }
  }
  ...
  /*componentDidUpdate () {
    ...
    if (query.from && query.to) {
      console.log('***** fitting to from/to');
      this.refs.map.leafletElement.fitBounds([
        [query.from.lat, query.from.lon],
        [query.to.lat, query.to.lon]
      ], {
        padding: [50, 50]
      })
    }
  ...
  render () {
    ...
    React.Children.toArray(children).forEach(child => {
      if (child.props.controlName) {
        ...
        controlledChildren.push(childWithVisibility)
      } else {
        uncontrolledChildren.push(child)
      }
    })
    ...
    return (
      ...
      >
        ...
        <LayersControl position='topright'>
          ...
          controlledChildren.map((child, i) => {
            return (
              <LayersControl.Overlay name={child.props.controlName} key={i}>
                {child}
