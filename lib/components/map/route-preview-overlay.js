...
import { getActiveSearch, getVisibleItineraryIndex } from '../../util/state'
...
const mapStateToProps = (state, ownProps) => {
  ...
  // Only show this overlay if the metro UI is explictly enabled
  if (config.itinerary?.showFirstResultByDefault !== false) {
    return {}
  }
  if (!activeSearchId) return {}

  const activeItinerary = getVisibleItineraryIndex(state)
  ...
  return {
    ...
    leafletPath: {
      color: '#333',
      dashArray: '3, 8',
      // TODO: should this overlay be hidden entirely when a route is hovered?
      opacity: activeItinerary === null ? 0.5 : 0,
      weight: activeItinerary === null ? 4 : 3
    }
  }
}
