...
class LegDiagram extends Component {
  ...
  _onResize = (width, height) => {
    const { traversed } = this._getElevationProfile()
    if (traversed > 0) {
      ...
      this.setState({ xAxisCompression })
    }
  }
  ...
  render () {
    ...
    const { minElev, maxElev, points, traversed } = this._getElevationProfile()
    ...
    // console.log({ traversed, width, xAxisCompression })
    return (
