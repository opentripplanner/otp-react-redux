...
class TransitiveOverlay extends MapLayer {
  ...
  componentDidMount () {
    ...
    map.on('movestart', (evt) => {
      ...
      this._my = 0
      this._moveSteps = 0
      this._mb = map.getPixelBounds()
    })

    map.on('move', (evt, q) => {
      console.log('    ---- MOVE', evt, q);
      this._moveSteps++
      if (evt.originalEvent) {
        //this._mx += evt.originalEvent.movementX
        //this._my += evt.originalEvent.movementY
    ...
    map.on('moveend', (evt) => {
      console.log('---- END', evt);
      console.log('   * steps', this._moveSteps);

      // calculate the pan-based offsets
      const pxBounds = map.getPixelBounds()
      //if (this._zoomed || this._moveSteps > 1) {
        this._pdx = pxBounds.min.x - this._initialPixelBounds.min.x
        this._pdy = pxBounds.min.y - this._initialPixelBounds.min.y
      //}
      ...
      /*if(!this._zoomed && this._moveSteps <= 1) {
        console.log('--------------------------------------------');
        const dx = pxBounds.min.x - this._mb.min.x
        const dy = pxBounds.min.y - this._mb.min.y
        this._zdx -= dx // -= 550
        this._zdy -= dy // += 750
      }*/
      this._zoomed = false
      console.log('   * moveend', this._zdx,this._zdy, this._pdx, this._pdy);
      //console.log(' -- mx/y', this._mx, this._my);
  ...
  _redraw () {
    ...
    console.log('*** updating trn bounds', mapBounds);
