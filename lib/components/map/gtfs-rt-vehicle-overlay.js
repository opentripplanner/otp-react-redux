...
function gtfsRtToTransitVehicle (gtfsRtVehicle, routes) {
  ...
  let routeShortName
  if (routes.length !== 0) {
    routeShortName = routes.find(r => r.route_id === trip.routeId)?.route_short_name
  }
  return {
 ...
 * and associated route information.
 */
class GtfsRtVehicleOverlay extends MapLayer {
   ...
   */
  _fetchRoutes = async () => {
    if (this.props.routeDefinitionUrl) {
      try {
        const response = await fetch(this.props.routeDefinitionUrl)
        if (response.status >= 400) {
          const error = new Error('Received error from server')
          error.response = response
          throw error
  ...
  render () {
    const { vehicleLocations, vehicleSymbols, visible } = this.state
    return (
