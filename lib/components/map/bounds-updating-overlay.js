...
class BoundsUpdatingOverlay extends MapLayer {
  ...
  updateBounds (oldProps, newProps) {
    ...
    // Also refit map if stateData prop has changed
    const stateDataChanged = !isEqual(oldProps.stateData, newProps.stateData)
    ...
    } else if (stateDataChanged) {
      // If a new stateData prop is passed,
      // force a resize of the map, then refit the active itinerary or active leg.
