    ...
    return (
      <S.NarrativeItinerariesContainer
        className={`options itinerary ${
          customBatchUiBackground && 'base-color-bg'
        }`}
      >
        ...
        >
          {showingErrors || mergedItineraries.length === 0 ? (
            ...
            <>
              {mergedItineraries.map((itinerary, index) => {
                const active = itinerary.index === activeItinerary
                // Hide non-active itineraries.
                if (!active && itineraryIsExpanded) return null

                // TODO: insert itinerary description only at the start of
                // a new "group"

                // groups are created via a reducer moving things into a modeDesc->itin structure.
                // that way sorting is preserved and things move around when needed

                // TODO: non-transit modes need to be sorted out elsewhere
                return (
                  <span key={itinerary.index}>
                    ...
                    <ItineraryBody
                      ...
                      LegIcon={LegIcon}
                      onClick={
                        active ? this._toggleDetailedItinerary : undefined
                      }
...
const mapStateToProps = (state) => {
  ...
  const { customBatchUiBackground, showHeaderText } = state.otp.config.itinerary
