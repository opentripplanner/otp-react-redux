    ...
    if (duplicateIndex === -1 || cur.legs.some(isFlex)) {
      ...
      if (!duplicateItin.allStartTimes) {
        duplicateItin.allStartTimes = [
          {
            itineraryIndex: duplicateItin.index,
  ...
  const allItineraries = itineraries.map((itin, index) => {
    return {
      ...
      allStartTimes: mergedItineraries.find((itn) =>
        itn.allStartTimes?.find((st) => st.itineraryIndex === index)
      )?.allStartTimes,
      index
    }
  })
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        >
          {showingErrors || itinerariesWithCo2.length === 0 ? (
            ...
            <>
              {groupItineraries && !itineraryIsExpanded ? (
                ...
                // This case is for the exapnded view of one itinerary.
                <ListContainer>{this._renderActiveItinerary()}</ListContainer>
              )}
...
const mapStateToProps = (state) => {
  ...
  let mergedItineraries = itineraries.map((itin, index) => ({ ...itin, index }))
  let allItineraries = itineraries
  if (mergeItineraries) {
    // eslint-disable-next-line prettier/prettier
    ({ allItineraries, mergedItineraries } = doMergeItineraries(itineraries))
  }

  const itinerary = itineraries?.[activeItinerary]
  const baselineCo2 = computeCarbonBaseline(itineraries, co2)
  const itinerariesWithCo2 =
    // eslint-disable-next-line prettier/prettier
    mergedItineraries?.map((itin) => addCarbonInfo(itin, co2, baselineCo2)) || []
