  ...
  render() {
    ...
    const groupedMergedItineraries = mergedItineraries.reduce(
      (prev, cur) => {
        ...
        const configModeOverrides = Object.values(routeModeOverrides)

        // Identify whether an itinerary uses transit & sort into appropriate bucket
        const transitLegs = cur.legs.filter(
          (leg) => isTransit(leg.mode) | configModeOverrides.includes(leg.mode)
        )
...
const mapStateToProps = (state) => {
  ...
  const { co2, errorMessages, modes, routeModeOverrides } = state.otp.config
