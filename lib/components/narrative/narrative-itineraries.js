    ...
    const mergedItineraries = itineraries.reduce((prev, cur, curIndex) => {
      ...
      if (duplicateIndex === -1) {
        ...
        if (!duplicateItin.allStartTimes) {
          ...
          duplicateItin.allStartTimes.add({
            realtime: containsRealtimeLeg(duplicateItin),
            time: duplicateItin.startTime
          })
        }
        // Ensure that the lowest start time is always first
        duplicateItin.allStartTimes.add({
          realtime: containsRealtimeLeg(cur),
          time: cur.startTime
        })

        // Some legs will be the same, but not share a route
        ...
        duplicateItin.legs = duplicateItin.legs.map((leg, index) => {
          ...
          if (leg?.routeId !== cur.legs[index]?.routeId) {
            ...
            const { agencyId, mode, routeColor, routeId, routeShortName } =
              cur.legs?.[index]
    ...
    const groupedMergedItineraries = mergedItineraries
      ...
      .reduce(
        (prev, cur) => {
          ...
          const modeContainer =
            // FIXME: A cleaner split is needed here
