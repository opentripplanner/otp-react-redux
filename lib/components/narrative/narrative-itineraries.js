...
class NarrativeItineraries extends Component {
  ...
  render() {
    ...
    let nextIndex = 0
    const mergedItineraries = itineraries.reduce((prev, cur) => {
      const updated = clone(prev)
      cur.index = nextIndex++

      const dupe = updated.findIndex((itin) => itinerariesAreEqual(itin, cur))
      // If no duplicate, push full itinerary to output
      if (dupe === -1) {
        updated.push(cur)
      } else {
        // Add only new start time to existing itinerary
        if (!updated[dupe].extraStartTimes) {
          updated[dupe].extraStartTimes = new Set()
        }

        // Ensure that the lowest start time is always first
        if (updated[dupe].startTime > cur.startTime) {
          updated[dupe].extraStartTimes.add(updated[dupe].startTime)
          updated[dupe].startTime = cur.startTime
        } else {
          updated[dupe].extraStartTimes.add(cur.startTime)
        }
      }
      return updated
    }, [])
