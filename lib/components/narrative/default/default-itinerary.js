...
const ITINERARY_ATTRIBUTES = [
  ...
  },
  {
    id: 'carbonEmission',
    order: 4,
    render: (itinerary, { co2Cutoff, co2Enabled }) => {
      if (!co2Enabled) return
      ...
      if (Math.abs(co2VsBaseline) <= co2Cutoff) return // Only show greater than the cutoff
      return (
        <>
          <FormattedNumber
            style="unit"
          ...
          <ItinerarySummaryWrapper>
            ...
            <ul className="list-unstyled itinerary-attributes">
              {ITINERARY_ATTRIBUTES.sort((a, b) => {
                ...
                if (
                  co2Config?.showCo2IfHigher ||
                  (itinerary.co2VsBaseline < 0 && co2Config?.enabled)
                ) {
                  options.co2Cutoff = co2Config.co2Cutoff || 0
                }
                return (
                  <li
