...
const ITINERARY_ATTRIBUTES = [
  ...
  },
  {
    ...
    render: (itinerary, { co2Config }) => {
      const { co2Cutoff, enabled, showCo2IfHigher } = co2Config
      if (!enabled) return
      const { co2VsBaseline } = itinerary
      if (!showCo2IfHigher && co2VsBaseline > 0) return
      const ROUND_TO_NEAREST = 5
      const co2VsBaselineRounded =
        Math.round((co2VsBaseline * 100) / ROUND_TO_NEAREST) * ROUND_TO_NEAREST
      if (Math.abs(co2VsBaseline * 100) <= co2Cutoff) return // Only show greater than the cutoff
      return (
        <>
          <FormattedMessage
