...
function ItineraryDescription({ itinerary }) {
  ...
  itinerary.legs.forEach((leg, i) => {
    ...
    if (isTransit(mode) && duration > primaryTransitDuration) {
      ...
      if (TransitModes) {
        // If custom transit modes have been defined for the given mode/leg, attempt to use them.
        transitMode = <TransitModes leg={leg} />
      } else {
        // If no custom transit mode has been defined, fall back on the ones defined below.
        transitMode = <FormattedMode mode={mode} />
      }
    }
