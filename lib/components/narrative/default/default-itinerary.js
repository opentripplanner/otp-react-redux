...
const ITINERARY_ATTRIBUTES = [
  ...
  },
  {
    ...
    render: (itinerary, { co2VsBaseline, hideCo2IfHigher }) => {
      ...
      co2VsBaseline = Math.round(co2VsBaseline * 100 / ROUND_TO_NEAREST)*ROUND_TO_NEAREST
      if(Math.abs(co2VsBaseline) <= 10) return // Only show greater than 10%
      return (
        <>
          <FormattedNumber
            style="unit"
          ...
          <ItinerarySummaryWrapper>
            ...
            <ul className='list-unstyled itinerary-attributes'>
              {ITINERARY_ATTRIBUTES.sort((a, b) => {
                ...
                if(co2Config?.showCo2IfHigher || co2VsBaseline < 0 && co2Config?.enabled) {
