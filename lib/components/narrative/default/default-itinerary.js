...
function ItineraryDescription ({itinerary}) {
  ...
  const mainMode = <FormattedMessage
    id={`common.accessModes.${accessModeId}`}
  />
  return transitMode
    ? <FormattedMessage
...
function FormattedTime ({startTime, endTime, timeFormat}) {
  return (
    <FormattedMessage
...
const ITINERARY_ATTRIBUTES = [
  {
    ...
    id: 'duration',
    order: 0,
    render: (itinerary, options) => (<FormattedDuration
