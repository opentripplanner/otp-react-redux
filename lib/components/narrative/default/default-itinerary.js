...
function getItineraryDescription (itinerary) {
  ...
  itinerary.legs.forEach((leg, i) => {
    ...
    if (isTransit(mode) && duration > primaryTransitDuration) {
      ...
      transitMode = <FormattedMessage id={mode} />
    }
    if (isBicycle(mode)) mainModeStringId = 'bike'
    if (isMicromobility(mode)) mainModeStringId = 'micromobility'
    if (rentedVehicle) mainModeStringId = 'micromobilityRent'
    if (rentedBike) mainModeStringId = 'bikeshare'
    if (mode === 'CAR') mainModeStringId = 'drive'
  })

  const mainMode = <FormattedMessage id={mainModeStringId} />
  return transitMode
...
class DefaultItinerary extends NarrativeItinerary {
  ...
  render () {
    ...
    const componentMessages = getMessages(['components', 'DefaultItinerary'], locale)
    const accessModes = getMessages(['common', 'accessModes'], locale)
    const otpTransitModes = getMessages(['common', 'otpTransitModes'], locale)
    const allMessages = {...componentMessages, ...otpTransitModes, ...accessModes}

    return (
      <IntlProvider locale={locale} messages={allMessages} defaultLocale='en-US'>
        <div
