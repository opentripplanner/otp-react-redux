...
function ItineraryDescription({ itinerary }) {
  ...
  return transitMode
    ? <FormattedMessage
      ...
      values={{ accessMode: mainMode, transitMode, primaryRoute }}
    />
    : mainMode
}
...
const ITINERARY_ATTRIBUTES = [
  ...
  },
  {
    ...
    render: (itinerary, options) => {
      ...
      if (itinerary.extraStartTimes) {
        const extraStartTimes = Array.from(itinerary.extraStartTimes)
        return (
          <FormattedMessage
            id='common.time.multiDepartureTimes'
            values={{
              startTime: formattedStartTime,
              secondStartTime: extraStartTimes?.[0] + options.offset || 0,
              thirdStartTime: extraStartTimes?.[1] + options.offset || 0
            }}
          />
        ) 
      }
