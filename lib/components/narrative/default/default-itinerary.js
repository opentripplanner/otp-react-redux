...
const { isBicycle, isMicromobility, isTransit, isFlex, isOptional, isContinuousDropoff, calculateEmissions } = coreUtils.itinerary
...
const ITINERARY_ATTRIBUTES = [
  ...
  },
  {
    ...
    render: (itinerary, { co2Config }) => {
      if(!co2Config) return // If there is no CO2 Config, don't display this element

      const { carbonIntensity, massUnit } = co2Config
      const co2 = calculateEmissions(carbonIntensity, massUnit, itinerary)
      return (
        <>
          <FormattedNumber
