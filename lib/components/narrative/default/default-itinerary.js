...
function ItineraryDescription({ itinerary }) {
  ...
  itinerary.legs.forEach((leg, i) => {
    ...
    if (isTransit(mode) && duration > primaryTransitDuration) {
      ...
      transitMode = (
        <FormattedMessage id={`common.otpTransitModes.${mode.toLowerCase()}`} />
      )
    }
  ...
  const mainMode = (
    <FormattedMessage id={`common.accessModes.${accessModeId}`} />
  )
  return transitMode ? (
    <FormattedMessage
...
class DefaultItinerary extends NarrativeItinerary {
  ...
  render() {
    ...
    const isCallAhead = itinerary.legs.some(isOptional)
    const isContinuousStops = itinerary.legs.some(isContinuousDropoff)
    ...
    return (
      ...
      >
        ...
        >
          ...
          {isFlexItinerary && <FlexIndicator isCallAhead={isCallAhead} isContinuousDropoff={isContinuousStops} shrink={active && !expanded} />}
