...
class ItineraryCarousel extends Component {
  ...
  /**
   * Prevent touch event from propogating through to nested SwipeableViews
   * component (horizontal carousel) and causing issues. See
   * https://github.com/oliviertassinari/react-swipeable-views/issues/418
   */
  _onTouchStart = (e) => {
  ...
  render () {
    const { itineraries, itineraryClass, pending, showProfileSummary } = this.props
    if (pending) return <Loading style={{ marginTop: '20px' }} />
    ...
    return (
      <SwipeableBottomSheet
        ...
        style={{ zIndex: 2000 }} // ensure this renders above map
        bodyStyle={{ zIndex: 2000 }} // ensure this renders above map
        overlay={false}
        marginTop={102} // keeps navbar/location summary visible
        overflowHeight={145}>
        <div
