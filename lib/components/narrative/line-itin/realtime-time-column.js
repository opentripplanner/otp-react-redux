...
function RealtimeTimeColumn ({
  ...
  const adjustedTime = time && moment(time).valueOf() + timeOptions.offset
  ...
  if (!isTransitLeg) {
    return (<div><FormattedTime timeFormat='short' value={adjustedTime} /></div>)
  }

  const delaySeconds = isDestination ? leg.arrivalDelay : leg.departureDelay
  const originalTimeMillis = time - delaySeconds * 1000
  const adjustedOriginalTime = moment(originalTimeMillis).valueOf() + timeOptions.offset
