...
function PlanFirstLastButtons(props) {
  const { enabled, itineraries, planFirst, planLast, planNext, planPrevious } =
    props
...
const mapStateToProps = (state) => {
  ...
  return {
    enabled:
      state.otp.config.itinerary?.showPlanFirstLastButtons &&
      state.callTaker.fieldTrip.visible === false,
    itineraries
  }
}
