...
import FavoritePlacePage from '../user/places/favorite-place-page'
  ...
  render () {
    ...
    const router = (
      <ConnectedRouter
        ...
        <div>
          <Switch>
            {/* TODO: Review the url schemes for consistency and brevity. */}
            <Route
              exact
              path={[
                // App root
            ...
            />
            <Route
              // id will be the index of the place, or 'new' if creating a new one.
              path={'/places/:id'}
              component={(routerProps) => {
                const props = this._combineProps(routerProps)
                return <FavoritePlacePage {...props} />
              }}
            />
            <Route
              path={'/savetrip'}
