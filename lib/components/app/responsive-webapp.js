...
import { loadLocaleData } from '../../util/i18n'
...
class ResponsiveWebapp extends Component {
  ...
  componentDidUpdate (prevProps) {
    ...
    // If the URL search parameters change (e.g., user modifies anything after ?, e.g. locale)
    // update the corresponding redux state.
    if (urlParams.locale && urlParams.locale !== prevProps.locale) {
      this.props.updateLocale(urlParams.locale)
    }
...
const mapDispatchToProps = {
  ...
  updateLocale: uiActions.updateLocale
}
...
class RouterWrapperWithAuth0 extends Component {
  ...
  componentDidUpdate (prevProps) {
    this.updateMessages(prevProps.locale)
  }

  async updateMessages (previousLocale) {
    // If the locale changes (or no prevProps are provided), update the messages
    // for that locale.
    const {customMessages, locale} = this.props
    if (locale && (!previousLocale || locale !== previousLocale)) {
      const messages = await loadLocaleData(locale, customMessages)
  ...
  render () {
    ...
    const router = (
      <ComponentContext.Provider value={components}>
        <IntlProvider locale={locale} messages={this.state.messages} defaultLocale={defaultLocale}>
          <ConnectedRouter
...
const mapStateToWrapperProps = (state, ownProps) => {
  const { defaultLocale, language, persistence, reactRouter } = state.otp.config
  return {
