...
class RouterWrapperWithAuth0 extends Component {
  ...
  async componentDidUpdate (prevProps) {
    this.updateMessages(prevProps)
  }

  async updateMessages (prevProps) {
    // If the locale changes (or no prevProps are provided), update the messages for that locale.
    const locale = this.props.locale
    if (locale && (!prevProps || locale !== prevProps.locale)) {
      this.setState({ messages: await loadLocaleData(locale, this.props.customMessages) })
    }
  }
  ...
  render () {
    ...
    const router = (
      <ComponentContext.Provider value={components}>
        <IntlProvider locale={locale} messages={this.state.messages} defaultLocale='en-US'>
          <ConnectedRouter
...
const mapStateToWrapperProps = (state, ownProps) => ({
  ...
  customMessages: state.otp.config.language,
  locale: state.otp.ui.locale,
