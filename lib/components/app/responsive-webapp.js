  ...
  componentDidUpdate(prevProps) {
    ...
    if (currentPosition !== prevProps.currentPosition) {
      ...
      if (
        ...
        const minlat = Math.min(query.from.lat, query.to.lat)
        const maxlat = Math.max(query.from.lat, query.to.lat)
        const minlon = Math.min(query.from.lon, query.to.lon)
        const maxlon = Math.max(query.from.lon, query.to.lon)
