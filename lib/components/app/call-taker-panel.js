...
import { generateCombinations } from '@opentripplanner/core-utils/lib/query-gen'
...
class CallTakerPanel extends Component {
  ...
  _planTrip = () => {
    ...
    // HACK: use `generateCombinations` to set the transit modes until
    // planConnection is implemented more correctly.
    // Can safely index 0, as we only ever have 1 mode with `input`
    if (currentQuery?.transitModes) {
      this.props.setQueryParam(
        generateCombinations({
          ...currentQuery,
          modes: [{ input: currentQuery.modes }, ...currentQuery?.transitModes],
          modeSettings: []
        })?.[0]
      )
    }

    // Reformat intermediate places for PlanConnection
    if (currentQuery?.intermediatePlaces) {
      this.props.setQueryParam({
        via: currentQuery.intermediatePlaces.map((ip) => ({
          coordinate: { latitude: ip.lat, longitude: ip.lon },
          label: ip.name
        }))
      })
    }
  ...
  render() {
    ...
    const { date, departArrive, from, intermediatePlaces, time, to } =
      currentQuery
    const { expandAdvanced, planTripClicked } = this.state
