...
const FormattedDateTimePreview = ({departArrive, endTime, routingType, startTime, time}) => {
  if (routingType === 'ITINERARY') {
    if (departArrive === 'NOW') {
      ...
      return <FormattedMessage id='components.DateTimePreview.arriveAt' values={{arriveTime: time}} />
    } else if (departArrive === 'DEPART') {
      return <FormattedMessage id='components.DateTimePreview.departAt' values={{departTime: time}} />
    }
  } else if (routingType === 'PROFILE') {
    return <FormattedMessage id='components.DateTimePreview.range' values={{endTime, startTime}} />
  }
}
