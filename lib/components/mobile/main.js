...
import { getActiveItinerary, getResponsesWithErrors } from '../../util/state'
...
class MobileMain extends Component {
  ...
  componentDidUpdate (prevProps) {
    ...
    // Display the results screen if results have been returned
    // (and an active itinerary has been set) or if there are errors.
    if (!prevProps.activeItinerary && (activeItinerary || errors.length > 0)) {
...
const mapStateToProps = (state, ownProps) => {
  const { config, currentQuery, location, ui } = state.otp
  return {
