      ...
      <ResultsMap expanded={mapExpanded}>
        <Map
          onClickLayerSelector={() => {
            ...
            // the map to ensure maximum visibility of layer options
            if (!mapExpanded) toggleBatchResultsMap()
          }}
          overrideNavigationControlPosition="top-left"
        />
      </ResultsMap>
      {hasErrorsAndNoResult ? (
        ...
        <NarrativeContainer
          className={`mobile-narrative-container ${
            itineraryExpanded ? 'itin-expanded' : ''
          }`}
          expanded={itineraryExpanded}
          visible={!mapExpanded}
        >
