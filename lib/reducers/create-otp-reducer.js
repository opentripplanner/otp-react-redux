...
function createOtpReducer (config, initialQuery) {
  ...
  const initialState = {
    ...
    searches: [],
    mapState: {
      bounds: latLngBounds(config.map.bounds)
    },
    activeSearch: 0
  }
  ...
  return (state = initialState, action) => {
    switch (action.type) {
      ...
      case 'PLAN_RESPONSE':
        ...
        // map geojson to each itinerary leg
        if (action.payload.plan) {
          action.payload.plan.itineraries.forEach((itinerary, itinIndex) => {
            itinerary.legs.forEach((leg, legIndex) => {
              leg.geojson = polyline.toGeoJSON(leg.legGeometry.points)
              leg.geojson.index = legIndex
            })
          })
        }
        return update(state, {
      ...
      case 'UPDATE_MAP_STATE':
        if (action.payload) {
          return update(state, { mapState: { $set: action.payload } })
        }
        return state
