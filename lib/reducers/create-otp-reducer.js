...
const { ensureSingleAccessMode, getDefaultQuery, getTripOptionsFromQuery } =
  coreUtils.query
...
export function getInitialState(userDefinedConfig) {
  ...
  if (config.locations) {
    locations.push(
      ...config.locations.map((l) => ({ ...l, type: 'suggested' }))
    )
  }
    ...
    switch (action.type) {
      ...
      case 'FORGET_PLACE': {
        ...
        if (action.payload.indexOf('recent') !== -1) {
          ...
          const removeIndex = recentPlaces.findIndex(
          ...
          return removeIndex !== -1
            ? update(state, {
                user: { recentPlaces: { $splice: [[removeIndex, 1]] } }
              })
            : state
        } else {
          const locations = clone(state.user.locations)
          const removeIndex = locations.findIndex(
            (l) => l.id === action.payload
          )
          removeItem(action.payload)
          return removeIndex !== -1
            ? update(state, {
                user: { locations: { $splice: [[removeIndex, 1]] } }
              })
            : state
        }
      }
      case 'REMEMBER_PLACE': {
        ...
        switch (type) {
          case 'recent': {
            const recentPlaces = clone(state.user.recentPlaces)
            const index = recentPlaces.findIndex((l) =>
              matchLatLon(l, location)
            )
            ...
            const sortedPlaces = recentPlaces.sort(
            ...
            storeItem('recent', recentPlaces)
            return update(state, {
              user: { recentPlaces: { $set: sortedPlaces } }
            })
          }
          default: {
            const locations = clone(state.user.locations)
      ...
      case 'FORGET_STOP': {
        ...
        const removeIndex = favoriteStops.findIndex(
          (l) => l.id === action.payload
        )
        ...
        return removeIndex !== -1
          ? update(state, {
              user: { favoriteStops: { $splice: [[removeIndex, 1]] } }
            })
          : state
      }
      case 'REMEMBER_STOP': {
        ...
        storeItem('favoriteStops', favoriteStops)
        return update(state, {
          user: { favoriteStops: { $set: favoriteStops } }
        })
      }
      // FIXME: set up action
      ...
      case 'REMEMBER_SEARCH':
        ...
        const duplicateIndex = searches.findIndex((s) =>
          isEqual(s.query, action.payload.query)
        )
      ...
      case 'TOGGLE_AUTO_REFRESH':
        ...
        return update(state, {
          user: { autoRefreshStopTimes: { $set: action.payload } }
        })
