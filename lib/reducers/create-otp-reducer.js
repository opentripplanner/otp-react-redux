...
export function getInitialState(userDefinedConfig) {
  ...
  const transitModeSettings = config?.modes?.transitModes.map((transitMode) => {
    const { gtfsMode, mode } = transitMode
    /**
     * If we're using custom route mode overrides, we pass both the
     * custom mode name (mode) and the gtfs mode to the modeSetting.
     */
    const modeSetting = gtfsMode || mode
    return {
      addTransportMode: { mode: modeSetting },
      applicableMode: 'TRANSIT',
      default: true,
      iconName: mode.toLowerCase(),
      key: mode.toLowerCase(),
      modeOverride: gtfsMode && mode,
      type: 'SUBMODE'
    }
  })
