...
function createOtpReducer(config) {
  ...
  return (prevState = initialState, action) => {
    ...
    switch (action.type) {
      ...
      case 'STOP_CLOSURES_RESPONSE':
        const alerts = action.payload.data.alerts
        const map = new Map()
        alerts.forEach((alert) => {
          const entities = alert.entities
          entities?.forEach((entity) => {
            if (entity.__typename === 'StopOnRoute') {
              const route = entity.route.gtfsId
              const stop = entity.stop.gtfsId
              map.set(route, (map.get(route) || new Set()).add(stop))
            }
          })
        })
        return update(state, {
      ...
      case 'TIMETABLE_ROUTE_RESPONSE':
        return update(state, {
          ui: {
            timetable: { $set: action.payload.data }
          }
        })
      case 'TIMETABLE_ROUTE_ERROR':
        return update(state, {
          ui: {
            timetable: {
              error: { $set: action.payload }
      ...
      case 'LOAD_MODULES':
        return update(state, {
          ui: {
            loadedModules: { $add: [...action.payload] }
          }
        })
      case 'LOADING_MESSAGES':
        return update(state, {
          ui: {
            loadingMessages: { $set: action.payload }
          }
        })
      default:
        return state
