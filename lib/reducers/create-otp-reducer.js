...
function createOtpReducer (config, initialQuery) {
  ...
  return (state = initialState, action) => {
    ...
    switch (action.type) {
      ...
      case 'SET_START_TIME':
        return update(state, { currentQuery: { startTime: { $set: action.payload.startTime } } })
      case 'SET_END_TIME':
        return update(state, { currentQuery: { endTime: { $set: action.payload.endTime } } })
      case 'SET_PLAN_TYPE':
        return update(state, { currentQuery: { type: { $set: action.payload.type } } })
      case 'SET_MAX_WALK_DISTANCE':
        return update(state, { currentQuery: { maxWalkDistance: { $set: action.payload.maxWalkDistance } } })
      case 'SET_OPTIMIZE':
        return update(state, { currentQuery: { optimize: { $set: action.payload.optimize } } })
      case 'FORM_CHANGED':
