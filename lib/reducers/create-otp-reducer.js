  ...
  return (prevState = initialState, action) => {
    ...
    switch (action.type) {
      ...
      case 'ROUTING_RESPONSE':
        ...
        // TODO: this should be handled on the server with actual otp routing options
        if (response.requestParameters.removeSingleModeItineraries) {
          response.plan.itineraries = response.plan.itineraries.filter(
            (itinerary) => {
              return !itinerary.legs.every((leg) => !leg.transitLeg)
            }
          )
        }
