...
function createCallTakerReducer (config) {
  ...
  return (state = initialState, action) => {
    switch (action.type) {
      ...
      case 'CLEAR_SAVABLE': {
        return update(state, {
          fieldTrip: { savable: { $set: {} } }
        })
      }
      case 'SET_SAVABLE': {
        // The payload represents whether the savable set of itineraries are for
        // the inbound or outbound journey.
        const savableUpdate = { [action.payload ? 'outbound' : 'inbound']: true }
        return update(state, {
          fieldTrip: { savable: { $set: savableUpdate } }
        })
      }
