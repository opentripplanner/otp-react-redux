 ...
 */
function rememberLocalUserPlace (location, duplicateFinder, beforeSave, state, fieldName, settingName) {
  ...
  if (duplicateIndex !== -1) places.splice(duplicateIndex, 1, location)
  else places.unshift(location)
  ...
  return (state = initialState, action) => {
    switch (action.type) {
      ...
      case 'REMEMBER_LOCAL_USER_PLACE': {
        ...
        switch (type) {
          case 'recent':
            return rememberLocalUserPlace(location, l => matchLatLon(l, location), sortAndTrim, state, 'recentPlaces', 'recent')
          default:
            storeItem(type, location)
            return rememberLocalUserPlace(location, l => l.type === type, null, state, 'savedLocations')
        }
      }

      case 'FORGET_STOP':
        return removeLocalUserPlace(action.payload.id, state, 'favoriteStops', 'favoriteStops')
      ...
      case 'FORGET_SEARCH':
        return removeLocalUserPlace(action.payload, state, 'recentSearches', 'recentSearches')

      case 'REMEMBER_SEARCH':
        return rememberLocalUserPlace(action.payload, s => isEqual(s.query, action.payload.query),
          sortAndTrim, state, 'recentSearches', 'recentSearches')
