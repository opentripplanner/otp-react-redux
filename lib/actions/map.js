...
import { findStopTimesForStop, routingQuery } from './api'
...
export function zoomToStop(stop) {
  return function (dispatch, getState) {
    if (!stop) return
    // Due to MapLibre changes, this is the only way to trigger a zoom.
    // the data being fed into the stops overlay must be updated!
