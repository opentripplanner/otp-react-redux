...
export function setLocationToCurrent(payload, intl) {
  return function (dispatch, getState) {
    const state = getState()
    const currentPosition = state.otp.location.currentPosition
    ...
    // Reverse geocode the current location to update the name
    getGeocoder(state.otp.config.geocoder)
      .reverse({ point: location })
      .then((geocodedLocation) => {
        if (geocodedLocation.name) {
          // Create a new location object with the geocoded name but keep everything else the same
          const updatedLocation = {
            ...location,
            name: geocodedLocation.name
          }

          // Update the location with the geocoded name
          dispatch(
            settingLocation({
              ...payload,
      ...
      .catch((err) => {
        // If reverse geocoding fails, we already have the default location set
        console.warn(err)
      })
  }
}
