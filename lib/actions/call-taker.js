...
const messages = defineMessages({
  ...
  couldNotFindCallError: {
    id: 'actions.callTaker.couldNotFindCallError'
  },
  fetchCallsError: {
...
export function endCall (intl) {
  return async function (dispatch, getState) {
    ...
    try {
      ...
      console.error(err)
      alert(intl.format(messages.callSaveError, { err: JSON.stringify(err) }))
    }
...
function newSession (datastoreUrl, verifyLoginUrl, redirect) {
  fetch(datastoreUrl + '/auth/newSession')
    ...
    .then(data => {
      ...
      const windowUrl = `${verifyLoginUrl}?${qs.stringify({redirect, session})}`
  ...
  return function (dispatch, getState) {
    fetch(datastoreUrl + `/auth/checkSession?sessionId=${sessionId}`)
      ...
      .catch(err => {
        ...
        alert(
          intl.format(messages.checkSessionError, { err: JSON.stringify(err) })
        )
      })
  }
}
...
export function fetchCalls (intl) {
  return async function (dispatch, getState) {
    ...
    try {
      ...
      alert(intl.format(messages.fetchCallsError, { err: JSON.stringify(err) }))
    }
  }
}
...
function saveQueriesForCall (call, intl) {
  return function (dispatch, getState) {
    ...
    if (sessionIsInvalid(callTaker.session)) return
    if (!call) {
      alert(intl.format(messages.couldNotFindCallError))
      return
    }
...
export function fetchQueries (callId, intl) {
  return function (dispatch, getState) {
    ...
    fetch(`${datastoreUrl}/calltaker/callQuery?sessionId=${sessionId}&call.id=${callId}`)
      ...
      .catch(err => {
        alert(
          intl.format(messages.queryFetchError, { err: JSON.stringify(err) })
        )
      })
  }
}
