...
export const clearSavable = createAction('CLEAR_SAVABLE')
export const setSavable = createAction('SET_SAVABLE')
 ...
 */
function makeFieldTripPlanRequests (request, outbound) {
  return async function (dispatch, getState) {
 ...
 */
export function deleteRequestTripItineraries (request, tripId) {
  return function (dispatch, getState) {
    const {callTaker, otp} = getState()
    const {datastoreUrl} = otp.config
    if (sessionIsInvalid(callTaker.session)) return
    ...
    return fetch(`${datastoreUrl}/fieldtrip/deleteTrip`,
      {method: 'POST', body: serialize({ id: tripId, sessionId })}
    )
      .then(() => dispatch(fetchFieldTripDetails(request.id)))
      .catch(err => {
        alert(`Error deleting field trip plan: ${JSON.stringify(err)}`)
      })
  }
}
