...
function makeSaveFieldTripItinerariesData (request, outbound, state) {
  ...
  // itierate through itineraries to construct itinerary and gtfsTrip data to
...
function getOTPLocationString (location) {
  const llString = `${location.lat},${location.lon}`
  return location.name
    ? `${location.name}::${llString}`
    : llString
}
 ...
 * accommodate the field trip group.
 */
...
async function getTripIdsForTravelDate (dispatch, state, request) {
  ...
  const formattedTravelDate = getFormattedRequestTravelDate(
    request,
 ...
 * itinerary according to the capacity avaiable in each itinerary.
 *
 ...
 * @return {Object} result.remainingGroupSize The remaining about of people in
...
/**
 * Formats a field trip's travel date into the given format.
 */
function getFormattedRequestTravelDate (
  request,
