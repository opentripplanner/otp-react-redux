...
export function deleteFieldTripNote(request, noteId, intl) {
  return function (dispatch, getState) {
    ...
    return fetch(`${datastoreUrl}/fieldtrip/deleteNote`, {
      body: serialize({ noteId, sessionId }),
      method: 'POST'
    })
      .then(() => dispatch(fetchFieldTripDetails(request.id, intl)))
      .catch((err) => {
 ...
 */
export function updateFieldTripRequest(
  request,
  endpoint,
  updatedFields,
  intl,
...
function makeSaveFieldTripItinerariesData(request, outbound, state) {
  ...
  const data = {
    ...
    trip: {
      ...
      departure: moment(getEarliestStartTime(itineraries)).format(
...
function prepareQueryParams(request, outbound) {
  return async function (dispatch, getState) {
    ...
    if (outbound) {
      ...
      queryParams.time = moment(request.arriveDestinationTime).format(
        OTP_API_TIME_FORMAT
      )
    } else {
      locationsToGeocode = {
      ...
      queryParams.time = moment(request.leaveDestinationTime).format(
        OTP_API_TIME_FORMAT
      )
    }
...
function getTripIdsForTravelDate(request) {
  return async function (dispatch, getState) {
    ...
    const formattedTravelDate = moment(request.travelDate).format(
...
export function deleteRequestTripItineraries(request, tripId, intl) {
  return function (dispatch, getState) {
    ...
    return fetch(`${datastoreUrl}/fieldtrip/deleteTrip`, {
      body: serialize({ id: tripId, sessionId }),
      method: 'POST'
    })
      .then(() => dispatch(fetchFieldTripDetails(request.id, intl)))
      .catch((err) => {
...
export function setRequestDate(request, date, intl) {
  return updateFieldTripRequest(request, 'setRequestDate', { date }, intl, () =>
    intl.formatMessage({
