...
function makeSaveFieldTripItinerariesData(request, outbound, state) {
  ...
  itineraries.forEach((itinerary) => {
    // Don't save itineraries with no size
    if (!itinerary.fieldTripGroupSize) {
      return
    }
    ...
    itinerary.legs.filter(isTransitLeg).forEach((leg) => {
      ...
      const gtfsTrip = {
        ...
        fromStopIndex: leg.trip?.departureStoptime?.stopPosition,
        fromStopName: leg.from.name,
        headsign: leg.headsign,
        routeName,
        toStopIndex: leg.trip?.arrivalStoptime?.stopPosition,
  ...
  return async function (dispatch, getState) {
    ...
    const confirmLocationOverride =
      showRePlanPopup && hasExistingLocations
        ? window.confirm(
            'Overwrite sidebar? Pressing "OK" will use sidebar plan. Pressing "Cancel" will overwrite'
          )
        : false
    ...
    const locations = !confirmLocationOverride
      ...
      : {
          departArrive: currentQuery.departArrive,
          from: currentQuery.from,
          time: currentQuery.time,
          to: currentQuery.to
        }
    return dispatch(setQueryParam({ ...queryParams, ...locations }))
  }
}
...
function tripsOverlap(leg, tripHashLookup, tripId, tripInUse) {
  ...
  if (
    leg.trip?.departureStoptime?.stopPosition >= tripInUse.toStopIndex ||
    leg.trip?.arrivalStoptime?.stopPosition <= tripInUse.fromStopIndex
  ) {
