...
// serviceDate is a Date object
const timetableQuery = (gtfsId, serviceDate) => {
  // "end" parameter is exclusive, so we need a Date object for the following day as well
  ...
  {  
    route(id: "${gtfsId}") {
      patterns(serviceDates: {
        start: "${format(serviceDate, 'yyyy-MM-dd')}",
        end: "${format(nextDay, 'yyyy-MM-dd')}"
      }) {
        directionId,
        name,
        tripsForDate(serviceDate: "${format(serviceDate, 'yyyyMMdd')}") {
          blockId,
          stoptimesForDate(serviceDate: "${format(serviceDate, 'yyyyMMdd')}") {
            serviceDay,
            scheduledArrival,
            scheduledDeparture,
            pickupType,
...
export function getTimetableData(params) {
  const { gtfsId, serviceDate } = params
  return function (dispatch) {
    dispatch(
      createGraphQLQueryAction(
        timetableQuery(gtfsId, serviceDate),
        {},
        timetableResponse,
        timetableError,
        {}
