...
const stopClosuresQuery = `
  query StopClosures {
    alerts(effect: NO_SERVICE) {
      entities {
        __typename
        ... on StopOnRoute {
          route {
...
const stopClosuresError = createAction('STOP_CLOSURES_ERROR')
const stopClosuresResponse = createAction('STOP_CLOSURES_RESPONSE')

export function getStopClosures() {
  return function (dispatch) {
    dispatch(
      createGraphQLQueryAction(
        stopClosuresQuery,
        {},
        stopClosuresResponse,
        stopClosuresError,
        {}
      )
    )
...
const timetableResponse = createAction('TIMETABLE_ROUTE_RESPONSE')
const timetableError = createAction('TIMETABLE_ROUTE_ERROR')

const timetableQuery = (date, routeGtfsId) => {
  const nextDay = new Date(date)
  nextDay.setDate(nextDay.getDate() + 1)
  const start = format(date, 'yyyy-MM-dd')
  const end = format(nextDay, 'yyyy-MM-dd')
  const serviceDate = format(date, 'yyyyMMdd')

  return `
  {  
    route(id: "${routeGtfsId}") {
      url
      shortName
      desc
      patterns(serviceDates: {
        start: "${start}",
        end: "${end}"
      }) {
        directionId,
...
export function getTimetableData(params) {
  const { date, routeGtfsId } = params

  return function (dispatch) {
    dispatch(
      createGraphQLQueryAction(
        timetableQuery(date, routeGtfsId),
        {},
        timetableResponse,
        timetableError,
        {}
...
export function routingQuery(searchId = null, updateSearchInReducer) {
  ...
  return function (dispatch, getState) {
    ...
    const { date, departArrive, modes, numItineraries, routingType, time } =
      currentQuery
    ...
    const excludedRoutes =
      config.routeModeOverrides &&
      getBannedRoutesFromSubmodes(modeSettings, config.routeModeOverrides)

    const { omitCanceledTrips = true } = config.itinerary || {}

    const baseQuery = {
      arriveBy,
      bannedRoutes: excludedRoutes || undefined,
      date,
    ...
    combinations.forEach((combo, index) => {
      const query = generateOtp2Query(combo, homeTimezone)
      dispatch(
        createGraphQLQueryAction(
          ...
          },
          {
            ...
            rewritePayload: (response, dispatch, getState) => {
              ...
              /**
               * ExtraTransferLegOnSameStop feature means that wait times on transfer legs are being added to the total walk time.
               * Account for this by compiling total walk time excluding transfer legs.
               */

              const itinerariesWithAdjustedWalkTimes = filteredItineraries.map(
                (itin) => ({
                  ...itin,
                  walkTime: itin.legs
                    .filter((l) => l.mode === 'WALK')
                    .filter((l) => l.duration > 0 && l.distance > 0)
              ...
              const withCollapsedShortNames =
                itinerariesWithAdjustedWalkTimes?.map((itin) => ({
                  ...itin,
                  legs: itin.legs
                    ?.map((leg) => {
                      ...
                      const routeColor = getRouteColorBasedOnSettings(
                        routeOperator,
                        routeProperties
                      )
              ...
              response.data.planConnection.routingErrors =
                response.data?.planConnection?.routingErrors.filter(
                  (re) => re?.code !== 'NO_TRANSIT_CONNECTION'
                )
              if (
                withCollapsedShortNames.length > 0 &&
                response?.data?.planConnection
              ) {
                if (!response.data.planConnection?.routingErrors) {
                  response.data.planConnection.routingErrors = []
                }
                ...
                if (displayA11yError) {
                  if (
                    ...
                    response.data.planConnection.routingErrors.push({
                ...
                if (displayTripInPastError !== false) {
                  if (
                    response.data.planConnection.edges.some(
                      (n) => n.node.endTime < Date.now()
                    )
                  ) {
                    response.data.planConnection.routingErrors.push({
