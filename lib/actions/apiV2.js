...
const getLegsWithCollapsedRouteNames = (legs) =>
  legs.map((leg) => ({
    ...leg,
    from: {
      ...leg.from,
...
export function routingQuery(searchId = null, combinations = [['TRANSIT']]) {
  return function (dispatch, getState) {
    const state = getState()
        ...
        routingError,
        {
          rewritePayload: (response, dispatch, getState) => {
            const withCollapsedShortNames = response.data.plan.itineraries.map(
              (itin) => ({
                ...itin,
                legs: getLegsWithCollapsedRouteNames(itin.legs),
                walkTime: getWalkTime(itin)
              })
            )
            const newResponse = {
              response: {
