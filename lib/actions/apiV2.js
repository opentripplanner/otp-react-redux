        ...
        createGraphQLQueryAction(
          ...
          },
          {
            ...
            rewritePayload: (response, dispatch, getState) => {
               ...
               * Account for this by identifying transfer legs and subtracting them from the total walk time.
               */

              const itinerariesWithAdjustedWalkTimes = filteredItineraries.map(
                (itin) => {
                  const transferLegsDuration = itin.legs
                    .filter((l) => l.distance === 0 && l.duration > 0)
                    .reduce((a, c) => a + c.duration, 0)
                  const adjustedWalkTime = itin.walkTime - transferLegsDuration

                  return { ...itin, walkTime: adjustedWalkTime }
                }
              )
