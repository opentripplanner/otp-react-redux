...
function formatRecentSearch(modeButtons, modeSettings, state) {
  return {
    id: randId(),
    query: getTripOptionsFromQuery(
      { ...state.otp.currentQuery, modeButtons, modeSettings },
      true
    ),
    timestamp: new Date().getTime()
  }
...
export function routingQuery(searchId = null, updateSearchInReducer) {
  return function (dispatch, getState) {
    ...
    const { date, departArrive, routingType, time } = currentQuery
    ...
    const activeModeKeys =
      ...
      }).modeButtons || config.modes.initialState.enabledModeButtons
    ...
    const modeSettingValues = state.otp.modeSettingDefinitions.reduce(
      (acc, setting) => {
        ...
        acc[setting.key] = fromUrl
          ...
          : config.modes.initialState.modeSettingValues?.[setting.key] ||
            setting.default
        return acc
      },
      {}
    ...
    const rawModeSettingQP = urlSearchParams.get('modeSettings')
    ...
    const combinations = generateCombinations({
      ...
      modes: activeModes,
      modeSettings,
      numItineraries: config?.modes?.numItineraries || 7,
      time,
      to: currentQuery.to
    })
        ...
        createGraphQLQueryAction(
          ...
          (response) => {
            ...
            if (
              ...
              dispatch(
                rememberSearch(
                  formatRecentSearch(rawModeButtonQP, rawModeSettingQP, state)
                )
              )
            }
