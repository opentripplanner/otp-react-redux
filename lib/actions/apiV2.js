...
/**
 * Merges stops that have the same stop code
 * TODO: find a better way to do this?
 */
const mergeSameStops = (nearbyResponse) => {
  const stopMap = new Map()

  nearbyResponse?.forEach(({ node }) => {
    const { code } = node.place
    if (code) {
      const existingStop = stopMap.get(code)
      if (existingStop) {
...
export const fetchNearby = (position, map) => {
  ...
  return createGraphQLQueryAction(
    ...
    {
      postprocess: (payload, dispatch) => {
        dispatch(zoomToPlace(map, position, 10))
      },
      rewritePayload: (payload) => {
