...
const { generateOtp2Query } = coreUtils.queryGen
...
export function routingQuery(searchId = null, updateSearchInReducer, modes) {
  return function (dispatch, getState) {
    const state = getState()
    ...
    const { routingType, wheelchair: accessibleRoutingEnabled } = currentQuery

    // if (!modes) {
    const activatedModes = getActivatedModesFromQueryParams(
      state.router.location.search,
      state.otp.config.modes.modeButtons,
      state.otp.config.modes.initialState
    )
    console.log('modes from url')
    console.log(activatedModes)
    // }

    const query = generateOtp2Query({
      from: currentQuery.from,
      modes: activatedModes,
      to: currentQuery.to
    })
    dispatch(
      routingRequest({
      ...
      createGraphQLQueryAction(
        ...
        routingError,
        {
          rewritePayload: (response, dispatch, getState) => {
            const withCollapsedShortNames = response.data?.plan.itineraries.map(
              (itin) => ({
                ...itin,
    ...
    if (isNewSearch || params.ui_activeSearch !== searchId) {
      console.log('new search id ' + searchId)
      console.log(modes)
