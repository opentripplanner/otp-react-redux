...
const { getTripOptionsFromQuery, getUrlParams, planParamsToQueryAsync } =
  coreUtils.query
...
/**
 * Deletes the specified query parameter from the current query and updates the
 * application state accordingly.
 */
export function deleteQueryParams(params) {
  return function (dispatch, getState) {
    const state = getState()
    ...
    dispatch(settingQueryParam(newQuery))
  }
}
