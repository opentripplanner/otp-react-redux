...
export function parseUrlQueryString (queryString) {
  return function (dispatch, getState) {
    ...
    // Store params in the current browser history state
    // FIXME: perhaps store this in redux store and never refer to window.history.state
    window.history.replaceState(params, '', queryString)
