...
export function routingQuery (searchId = null) {
  return async function (dispatch, getState) {
    ...
    //
    fetch(constructRoutingQuery(otpState, true), getUserFetchOptions(state))
      .then(getJsonAndCheckResponse)
      .then(json => {
 ...
 */
function getUserFetchOptions (state) {
  // The check for state.otpUser is needed for tests.
  if (state.otpUser) {
    const { config } = state.otp
    const { accessToken, loggedInUser } = state.otpUser
