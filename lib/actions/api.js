...
export function routingQuery (searchId = null) {
  return async function (dispatch, getState) {
    ...
    const otpUserState = getState().otpUser
    ...
    // only add the OTP user fetch options (including the Auhorization header)
    ...
    //
    fetch(constructRoutingQuery(otpState, true), getUserFetchOptions(otpState, otpUserState))
      .then(getJsonAndCheckResponse)
      .then(json => {
...
/**
 * Gets a non-blank fetch options object if all the following applies:
 * - The user is logged in (otpUserState.loggedInUser is not null),
 * - An accessToken for that user is stored (otpUserState.accessToken should be already set),
 * - A middleware API key has been set in the configuration,
 * - The user has consented to storeTripHistory.
 */
function getUserFetchOptions (otpState, otpUserState) {
  const { config } = otpState
  const { accessToken, loggedInUser } = otpUserState
  const { apiKey } = config.persistence.otp_middleware

  if (accessToken && loggedInUser && loggedInUser.storeTripHistory) {
    return getSecureFetchOptions(accessToken, apiKey)
  }
