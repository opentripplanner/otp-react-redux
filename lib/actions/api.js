...
export function routingQuery (customOtpQueryBuilder) {
  return async function (dispatch, getState) {
    ...
    dispatch(routingRequest({ routingType, requestId }))
    ...
    try {
      ...
      return dispatch(routingError({ error, requestId }))
    }

    dispatch(routingResponse({ response: json, requestId }))
  }
}
...
function constructRoutingQuery (otpState) {
  ...
  queryParams
    .filter(
      qp => qp.planTypes.indexOf(routingType) !== -1 && qp.name in currentQuery
    )
    .forEach(qp => {
