...
// FIXME: This file has many misplaced methods and variables
...
// Core-utils is prevent typescripting
...
function getOtpFetchOptions(state, includeToken = false) {
  ...
  if (persistence && persistence.otp_middleware) {
    // Prettier can't understand the syntax on the following line
...
function handleThrottlingUrl (url, fetchOptions) {
  const throttleKey = fetchOptions
    ? `${url}-${hash(fetchOptions)}`
    : url
