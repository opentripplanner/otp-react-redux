...
export function routingQuery(searchId = null, updateSearchInReducer = false) {
  return function (dispatch, getState) {
    ...
    // one iteration.
    const { wheelchair: accessibleRoutingEnabled } = state.otp.currentQuery
    const iterations = state.otp.currentQuery.combinations
      ? state.otp.currentQuery.combinations
          .map(({ mode, params }) => ({
            mode,
            ...params
          }))
          ...
          .filter((combination) => {
            ...
            // combinations without the pparameter set (undefined)
    ...
    return Promise.all(
      iterations.map((injectedParams, i) => {
        ...
        return Promise.all([realTimeFetch])
      })
    )
  }
}
