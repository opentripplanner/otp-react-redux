...
export function setUrlSearch(params, replaceCurrent = false) {
  return function (dispatch, getState) {
    ...
    const oldSearch = qs.parse(getState().router.location.search.split('?')[1])
...
export function updateOtpUrlParams(state, searchId) {
  ...
  return function (dispatch, getState) {
    ...
    // Populate the URL parameters for the mode buttons if those are specified in the currentQuery
    if (currentQuery.modeButtons) {
      params.modeButtons = currentQuery.modeButtons
    }
    if (currentQuery.modeSettings) {
      params.modeSettings = currentQuery.modeSettings
    }
