...
function constructPlanQuery (api, query) {
  ...
  let params = {}
  queryParams.filter(qp => qp.planTypes.indexOf('ITINERARY') !== -1 && qp.name in query).forEach(qp => {
    params = Object.assign(params, qp.itineraryRewrite
      ? qp.itineraryRewrite(query[qp.name])
...
function constructProfileQuery (api, query) {
  ...
  let params = {}
  queryParams.filter(qp => qp.planTypes.indexOf('PROFILE') !== -1 && qp.name in query).forEach(qp => {
    params = Object.assign(params, qp.profileRewrite
      ? qp.profileRewrite(query[qp.name])
      : { [qp.name]: query[qp.name] })
  })
