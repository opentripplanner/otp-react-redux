    ...
    iterations.forEach((injectedParams, i) => {
      ...
      fetch(query, getOtpFetchOptions(state))
        ...
        .then(json => {
          ...
          const trueUser = userState.loggedInUser || userState.localUser
          if (trueUser.storeTripHistory) {
      ...
      fetch(constructRoutingQuery(otpState, true), getOtpFetchOptions(state, storeTripHistory && i === 0))
        .then(getJsonAndCheckResponse)
        .then(json => {
