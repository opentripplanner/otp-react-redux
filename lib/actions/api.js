...
function getOtpFetchOptions(state, includeToken = false) {
  ...
  const { api: api_v1, api_v2, persistence } = state.otp.config
  ...
  return createQueryAction(
    ...
    findRoutesError,
    {
      ...
      rewritePayload: (payload, dispatch, getState) => {
        const { otp } = getState()
        // eslint-disable-next-line camelcase
        const { api: api_v1, api_v2 } = otp.config
        // eslint-disable-next-line camelcase
        const usingV2Only = !api_v1 && !!api_v2
        const routes = {}
        payload.forEach((rte) => {
...
function makeApiUrl(config, endpoint, options) {
  ...
  if (
    ...
    const api =
      ...
      (options.v2 && config.api_v2) || !config.api ? config.api_v2 : config.api
