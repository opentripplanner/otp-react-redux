...
function createQueryAction (endpoint, responseAction, errorAction, options = {}) {
  return async function (dispatch, getState) {
    ...
    let payload
    try {
      const response = await fetch(url, makeApiFetchOptions(config, options))
...
/**
 * Add the necessary API key headers if using an API Key.
 *
 * @param  {Object} config  The app-wide config
 * @param  {Object} options The options for the API request
 * @return {Object}         The options to send to the fetch function when
 *                      making the http request for the API endpoint.
 */
function makeApiFetchOptions (config, options) {
  const fetchOptions = options.fetchOptions || {}
  const apiKey = config?.persistence?.otp_middleware?.apiKey
  if (apiKey) {
