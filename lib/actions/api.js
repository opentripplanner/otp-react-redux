...
export function routingQuery (searchId = null) {
  return async function (dispatch, getState) {
    ...
    //
    fetch(constructRoutingQuery(otpState, true), getUserFetchOptions(otpState))
      .then(getJsonAndCheckResponse)
      .then(json => {
 ...
 * - An accessToken for that user is stored (otpState.user.accessToken should be already set),
 * - A middleware API key has been set in the configuration,
 * - The user has consented to storeTripHistory.
 */
function getUserFetchOptions (otpState) {
  const { config, user } = otpState
  if (user) {
    const { accessToken, loggedInUser } = user
    const { apiKey } = config.persistence.otp_middleware

    if (accessToken && loggedInUser) {
      const { storeTripHistory } = loggedInUser
      if (storeTripHistory) {
