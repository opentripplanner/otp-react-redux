...
import { convertToPlace, getPersistenceStrategy } from '../util/user'
...
export function routingQuery(searchId = null, updateSearchInReducer = false) {
  return function (dispatch, getState) {
    ...
    const persistenceStrategy = getPersistenceStrategy(
      state.otp.config.persistence
    )
    ...
    const iterations = combinations
      ? combinations.map(({ mode, params }) => ({ mode, ...params }))
      : [{}]
    dispatch(
      routingRequest({
        activeItinerary,
    ...
    return Promise.all(
      iterations.map((injectedParams, i) => {
        ...
        //
        const shouldStoreTrip = isUsingOtpMiddleware
