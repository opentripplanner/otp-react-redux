...
export function routingQuery (searchId = null) {
  return async function (dispatch, getState) {
    ...
    fetch(query, getMiddlewareFetchOptions(state))
    ...
    fetch(constructRoutingQuery(otpState, true), getMiddlewareFetchOptions(state, storeTripHistory))
      .then(getJsonAndCheckResponse)
      .then(json => {
...
/**
 * This method determines the fetch options and HTTP authentication headers
 * for OTP middleware requests that can optionally be authenticated.
 * The fetch options may include the following:
 ...
 * (Assumes JSON request bodies.)
 */
function getMiddlewareFetchOptions (state, includeToken = false) {
  let token = null
  let apiKey = null

  const { persistence } = state.otp.config
  if (persistence && persistence.otp_middleware) {
    apiKey = persistence.otp_middleware.apiKey
  }
