    ...
    findRoutesError,
    {
      ...
      rewritePayload: (payload, dispatch, getState) => {
        ...
        payload?.data?.routes?.forEach((rte) => {
          // Strip out the agency part of the route ID which can sometimes differ
          let strippedRouteId = rte.gtfsId.split(':')
          strippedRouteId = strippedRouteId[strippedRouteId.length - 1]
    ...
    if (route?.v2) {
      return dispatch(
        createGraphQLQueryAction(
          // TODO: cleanup stops query
          `{
            route(id: "${route.gtfsId}") {
              gtfsId
              desc
              agency {
                gtfsId
          ...
          findRouteError,
          {
            ...
            rewritePayload: (payload) => {
              ...
              // Re-write query to achieve otp-v1 compatibility
              route.id = route.gtfsId
              route.agency.id = route.agency.gtfsId
              route.routeBikesAllowed = route.bikesAllowed
...
export function getVehiclePositionsForRoute(routeId) {
  return function (dispatch, getState) {
    ...
    // If a route was fetched from GraphQL, it doens't support realtime vehicle positions
    if (route && route.v2) {
      return
    }
    return dispatch(
