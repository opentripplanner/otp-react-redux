  ...
  return function (dispatch, getState) {
    ...
    return Promise.all(
      iterations.map((injectedParams, i) => {
        ...
        //
        let shouldStoreTrip = false
        if (persistenceMode.isOtpMiddleware) {
          shouldStoreTrip = state.user?.loggedInUser?.storeTripHistory
        } else if (persistenceMode.isLocalStorage) {
          shouldStoreTrip = state.user?.localUser?.storeTripHistory
        }
        ...
        const nonRealtimeFetch = fetch(
          ...
          .then((json) => {
            ...
            if (persistenceMode.isOtpMiddleware && shouldStoreTrip) {
              setTimeout(() => {
