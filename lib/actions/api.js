...
/**
 * Gets a non-blank userId query parameter if all the following applies:
 * - The user is logged in (otpState.user.loggedInUser is not null),
 * - The user had an id attribute (should be always true),
 * - The user has consented to storeTripHistory.
 */
function makeUserIdParam (otpState) {
  const { user } = otpState
  if (user) {
    const { loggedInUser } = user
    if (loggedInUser) {
      const { id, storeTripHistory } = loggedInUser
      if (id && storeTripHistory) {
        return `&userId=${id}`
      }
    }
  }
...
function constructRoutingQuery (otpState, ignoreRealtimeUpdates) {
  ...
  // For each trip request, constructRouteQuery is called twice with identical queries,
  // except for the ignoreRealtimeUpdates parameter.
  //
  // To avoid recording the same trip request twice in the middleware,
  // only add the OTP user id to the routing query when
  // ignoreRealTimeUpdates === true.
  // The advantage of using ignoreRealTimeUpdates === true is that the middleware will be able to
  // record and provide the theoretical itinerary summary without having to query OTP again.
  const userIdParam = ignoreRealtimeUpdates ? makeUserIdParam(otpState) : ''

  return `${planEndpoint}?${qs.stringify(params)}${userIdParam}`
}
