...
import { fetchTripRequests, rememberPlace } from './user'
...
export function routingQuery(searchId = null, updateSearchInReducer = false) {
  return function (dispatch, getState) {
    ...
    const persistenceStrategy = getPersistenceStrategy(state.otp.config.persistence)
    ...
    const { combinations, routingType } = currentQuery
    ...
    return Promise.all(iterations.map((injectedParams, i) => {
      ...
      const realTimeFetch = fetch(url, getOtpFetchOptions(state))
        ...
        .then(json => {
          ...
          // search is completed.
          if (!isUsingOtpMiddleware && state.user?.localUser?.storeTripHistory) {
      ...
      //
      // If storeTripHistory is enabled, we only send the token for logging for the first request,
      // subsequent requests from the `iterations` variable are identical
      // (except for the mode combinations -- they are regenerated for every inquiry).
      //
      ...
      //
      const shouldStoreTrip = i === 0 && (
        isUsingOtpMiddleware
          ? state.user?.loggedInUser?.storeTripHistory
      ...
      const nonRealtimeFetch = fetch(
        ...
        getOtpFetchOptions(state, shouldStoreTrip)
      )
        .then(getJsonAndCheckResponse)
        .then(json => {
          // For OTP Middleware users, if tracking is enabled,
          // refetch recent trip requests after a few seconds,
          // so that persistence of trip requests has time to complete.
          if (isUsingOtpMiddleware && state.user?.loggedInUser?.storeTripHistory) {
            setTimeout(() => {
