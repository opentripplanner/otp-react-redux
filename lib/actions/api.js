    ...
    iterations.forEach((injectedParams, i) => {
      ...
      fetch(query, getOtpFetchOptions(state))
        ...
        .then(json => {
          ...
          // search is completed for the applicable user.
          const currentUser = userState.loggedInUser || userState.localUser
          if (currentUser.storeTripHistory) {
      ...
      //
      const { user } = state
      const loggedInUserStoreTripHistory = user &&
        user.loggedInUser &&
        user.loggedInUser.storeTripHistory

      fetch(constructRoutingQuery(otpState, true), getOtpFetchOptions(state, loggedInUserStoreTripHistory && i === 0))
        .then(getJsonAndCheckResponse)
        .then(json => {
