  ...
  return function (dispatch, getState) {
    ...
    switch (root) {
      ...
      case '@': {
        ...
        const [lat, lon, zoom, routerId] = id ? idToParams(id) : []
        if (!lat || !lon) {
...
export function setLocale(locale) {
  return async function (dispatch, getState) {
    ...
    // TODO: Should we get the locales from the config every
    // time we set the locale or should we destructure once into the
    // store.
