...
const loadModule = createAction('LOAD_MODULE')
...
export function isModuleLoaded(state, appModule) {
  const { loadedModules, loadingMessages = false } = state.otp.ui
  return loadedModules.includes(appModule) && !loadingMessages
}

export function loadAppModule(appModule) {
  return async function (dispatch, getState) {
    if (!isModuleLoaded(getState(), appModule)) {
      dispatch(loadingMessages(true))
      dispatch(loadModule(appModule))
      await dispatch(setLocale(getState().otp.ui.locale, false, true))
