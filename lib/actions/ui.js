...
export function setHighlightedStop (payload) {
  return function (dispatch, getState) {
    dispatch(setHoveredStop(payload))
  }
}

const setHoveredStop = createAction('SET_HOVERED_STOP')
...
export function setViewedRoute (payload) {
  return function (dispatch, getState) {
    ...
    const patternSuffix = payload?.patternId ? `/pattern/${payload.patternId}` : ''

    const path = payload?.routeId
      ? `/route/${payload.routeId}${patternSuffix}`
      : '/route'
    dispatch(routeTo(path))
  }
}
