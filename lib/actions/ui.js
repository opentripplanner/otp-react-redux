...
const loadModules = createAction('LOAD_MODULES')
const loadingMessages = createAction('LOADING_MESSAGES')
...
export function setLocale(
  ...
  return async function (dispatch, getState) {
    ...
    if (matchedLocale !== ui.locale || forceUpdate) {
...
export function areModulesLoaded(state, appModules) {
  const { loadedModules, loadingMessages = false } = state.otp.ui
  return (
    !loadingMessages &&
    appModules.every((appModule) => loadedModules.has(appModule))
  )
}

export function loadAppModules(appModules) {
  return async function (dispatch, getState) {
    const state = getState()
    if (!state.otp.ui.loadingMessages && !areModulesLoaded(state, appModules)) {
      dispatch(loadingMessages(true))
      dispatch(loadModules(appModules))
      await dispatch(setLocale(state.otp.ui.locale, false, true))
