...
/**
 * Ensures user monitored trips are loaded.
 * state.user.loggedInUserMonitoredTrips is at least an empty array when loaded.
 */
export function ensureUserMonitoredTripsAreLoaded () {
  return async function (dispatch, getState) {
    const { user } = getState()
    const { accessToken, loggedInUser, loggedInUserMonitoredTrips } = user
    if (accessToken && loggedInUser && !loggedInUserMonitoredTrips) {
