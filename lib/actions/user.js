  ...
  return async function (dispatch, getState) {
    ...
    if (otpMiddleware) {
      ...
      if (accessToken) {
        ...
        const { email_verified: isEmailVerified } = authUser
        ...
        if (!isNewAccount) {
          ...
          dispatch(setCurrentUser({ accessToken, isEmailVerified, user }))
        } else {
          dispatch(setCurrentUser({ accessToken, isEmailVerified, user: createNewUser(authUser) }))
        }
      }
    }
  }
