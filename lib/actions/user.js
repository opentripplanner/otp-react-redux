...
export function fetchTripRequests () {
  return async function (dispatch, getState) {
    ...
    //Is .then() better?
    //Also figure out whether to convert trip requests into search objects here.
    //Also figure out a way to deal with batch ids and remove requests of the same batch.
    if (emptyStatus === 'success') {
      ...
      if (status === 'success') {
        dispatch(setCurrentUserTripRequests(trips.data))
      }
    }
  }
}
...
export function deleteUserPlace (place) {
  return function (dispatch, getState) {
    ...
    const { loggedInUser } = getState().user
    const placeIndex = loggedInUser.savedLocations.indexOf(place)
