...
function convertRequestToSearch(config) {
  return function (tripRequest) {
    const { dateCreated, id, queryParams } = tripRequest
...
export function fetchTripRequests() {
  return async function (dispatch, getState) {
    ...
    if (emptyStatus === 'success') {
      const { data: trips, status } = await secureFetch(
        `${requestUrl}&offset=${requests.total - DEFAULT_LIMIT}`,
        accessToken,
        apiKey,
        'GET'
      )
      if (status === 'success') {
        ...
        const convertedTrips = trips.data.map(
          convertRequestToSearch(getState().otp.config)
        )
 ...
 */
function setUser(user, fetchTrips, intl) {
  return function (dispatch, getState) {
    positionHomeAndWorkFirst(
      user,
      intl.formatMessage({ id: 'common.places.home' }),
      intl.formatMessage({ id: 'common.places.work' })
    )
 ...
 * @param intl A formatjs intl object
 */
export function fetchOrInitializeUser(auth0User, intl) {
  return async function (dispatch, getState) {
    const { accessToken, apiBaseUrl, apiKey } = getMiddlewareVariables(
      getState()
    )
    ...
    dispatch(setUser(userData, !isNewAccount, intl))
  }
}
...
export function createOrUpdateUser(userData, silentOnSuccess = false, intl) {
  return async function (dispatch, getState) {
    ...
    if (status === 'success' && returnedUser) {
      ...
      // (This sorts saved places, and, for existing users, fetches trips.)
      dispatch(setUser(returnedUser, isCreatingUser, intl))
    } else {
      alert(
        intl.formatMessage(
 ...
 * @param intl A formatjs intl object
 */
export function deleteUser(userData, auth0, intl) {
  return async function (dispatch, getState) {
    const { accessToken, apiBaseUrl, apiKey } = getMiddlewareVariables(
      getState()
    )
  ...
  return async function (dispatch, getState) {
    ...
    if (number !== newPhoneNumber || now - timestamp >= 60000) {
      ...
      if (status === 'success') {
        ...
        dispatch(fetchOrInitializeUser(intl))
      } else {
        alert(
          intl.formatMessage(
            { id: 'actions.user.genericError' },
...
export function saveUserPlace(placeToSave, placeIndex, intl) {
  return function (dispatch, getState) {
    ...
    // FIXME_QBD Remove "blank" attribute, it is not recognized by the middleware.
    // const { blank, ...placeToSave } = place
 ...
 */
export function deleteUserPlace(place) {
  return function (dispatch, getState) {
