 ...
 */
function removeBatchDuplicateRequests(filtered, tripRequest) {
...
export function fetchTripRequests() {
  return async function (dispatch, getState) {
    ...
    // This call is intended to get the total number of entries, so that we only load the most recent ones.
    // This is currently so because in OTP-middleware, entries are kept and returned oldest first.
    // Previously, the limit was set to zero, however that resulted in all entries unexpectedly being downloaded.
    // Therefore, we set the limit to 1 to avoid a mass download on first load.
    ...
    // TODO:
    // 1. Make OTP-middleware return the most recent trip searches instead (this will avoid the call above).
    // 2. Make OTP-middleware not return duplicate search entries corresponding to the same search.
    ...
    if (emptyStatus === 'success') {
      ...
      if (status === 'success') {
        ...
        const convertedTrips = trips.data
          ...
          .reduce(removeBatchDuplicateRequests, [])

        dispatch(setCurrentUserTripRequests(tidyRecentSearches(convertedTrips)))
      }
    }
  }
}

export function applyUserSavedTripDefaults(userSavedTripDefaults) {
  return function (dispatch, getState) {
    const state = getState()
    const { mainPanelContent, mobileScreen } = state.otp.ui
    /* Only apply when we're looking at the trip form and if the user 
    isn't already in the middle of a search */
    if (
      !window.location.href.includes('activeSearch') &&
      (coreUtils.ui.isMobile()
        ? mobileScreen === MobileScreens.SEARCH_FORM
        : !mainPanelContent)
    ) {
      const userDefaultParams = JSON.parse(userSavedTripDefaults)
      dispatch(setQueryParam(userDefaultParams))
      dispatch(setUrlSearch(userDefaultParams, true))
    }
  }
}
...
function setUser(user, fetchTrips) {
  return function (dispatch, getState) {
    ...
    const hasMobilityProfile = !!state.otp.config.mobilityProfile
    ...
    if (accessibilityRoutingByDefault !== undefined && !userSavedTripDefaults) {
    ...
    if (
      hasMobilityProfile &&
      !state.router.location.pathname.startsWith('/account')
    ) {
      // TODO: Refactor below.
      ...
      // to render itself.
      // Note: The current search id, if any, is passed to prevent a view change
      // (e.g. to prevent navigating away from the trip viewer after reloading page).
      // FIXME: remove hack
      setTimeout(() => {
        dispatch(routingQuery(state.otp.activeSearchId))
      }, 300)
    }
  }
}
...
export function resendVerificationEmail(intl) {
  return async function (dispatch, getState) {
    ...
    await toastPromise(
      secureFetch(requestUrl, accessToken, apiKey),
...
export function createOrUpdateUserMonitoredTrip(
  ...
  return async function (dispatch, getState) {
    ...
    const successMessage = formattedToastSuccessMessage(
      isNew
        ? intl.formatMessage({
    ...
    const { data, message, status } = await toastPromise(
      secureFetch(requestUrl, accessToken, apiKey, method, {
        body: JSON.stringify(tripData)
      }),
      formattedToastSuccessMessage(successMessage),
      intl,
      id,
      silentOnSuccess
    )
 ...
 */
export function deleteLoggedInUserPlace(placeIndex, intl, placeToSave) {
  return function (dispatch, getState) {
    const { loggedInUser } = getState().user
    ...
    return toastPromise(
      dispatch(createOrUpdateUser(loggedInUser, intl)),
      toastMessageOnPlaceChanged(placeToSave, intl, 'Deleted'),
      intl
    )
  }
}
