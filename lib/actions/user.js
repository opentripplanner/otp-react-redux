...
function convertRequestToSearch(config) {
  return function (tripRequest) {
    const {
      ...
      id,
      otp2QueryParams = {},
      requestParameters = {} // Deprecated OTP1
    } = tripRequest
    const { host, path } = config.api

    // FIXME: remove if OTP-UI no longer includes trailing bracket
    // Don't use ?. syntax, fromPlace and toPlace need to be defined in order to be replaced.
    if (otp2QueryParams.fromPlace && otp2QueryParams.fromPlace.endsWith('}')) {
      otp2QueryParams.fromPlace = otp2QueryParams.fromPlace.substring(
        0,
        otp2QueryParams.fromPlace.length - 2
      )
    }
    if (otp2QueryParams.toPlace && otp2QueryParams.toPlace.endsWith('}')) {
      otp2QueryParams.toPlace = otp2QueryParams.toPlace.substring(
        0,
        otp2QueryParams.toPlace.length - 2
      )
    }
    ...
    return {
      canDelete: false,
      id,
      query: planParamsToQuery(otp2QueryParams || requestParameters || {}),
...
function removeDuplicateRequests(filtered, tripRequest) {
  ...
  if (filtered.length === 0) {
    ...
    !areRequestsSame(filtered[filtered.length - 1].query, tripRequest.query)
  ) {
    filtered.push(tripRequest)
  } else {
